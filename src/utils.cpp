...
std::string formatTime(double time) {
  ...
  if (hours > 0)
    return fmt::format("{}:{:02d}:{:02d}.{:03d}", hours, minutes, seconds,
                       millis);
  if (minutes > 0)
...
float getActualProgress(GJBaseGameLayer *game) {
  ...
  if (game->m_level->m_timestamp > 0) {
    percent = static_cast<float>(game->m_gameState.m_levelTime * 240.f) /
...
bool matchesStringFuzzy(std::string_view haystack, std::string_view needle) {
  auto it = std::ranges::search(haystack, needle, [](char ch1, char ch2) {
