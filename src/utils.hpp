...
enum class PlayerMode { Cube, Ship, Ball, UFO, Wave, Robot, Spider, Swing };
...
/// @example auto& member = utils::memberByOffset\<int>(ptr, offsetof(Struct,
/// member));\n geode::log::info("Member: {}", member);\n member = 42;
/// @warning Use with caution, for debugging purposes only.
template <typename T>
[[deprecated("Don't use MBO!")]] constexpr T &memberByOffset(void *ptr,
                                                             size_t offset) {
...
gui::Color getRainbowColor(float speed, float saturation, float value,
                           float offset = 0.f);

/// @brief Get the current player game mode. If player is nullptr, returns
