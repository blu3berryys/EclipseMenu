...
void RenderTexture::begin() {
...

  {
    std::unique_ptr<char, void (*)(void*)> data(
        static_cast<char*>(malloc(m_width * m_height * 4)), free);

    memset(data.get(), 0, m_width * m_height * 4);
    m_texture->initWithData(data.get(),
                            cocos2d::kCCTexture2DPixelFormat_RGBA8888, m_width,
                            m_height,
                            cocos2d::CCSize(static_cast<float>(m_width),
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0_EXT,
                         GL_TEXTURE_2D, m_texture->getName(), 0);
...
void RenderTexture::end() const { m_texture->release(); }

void RenderTexture::capture(std::mutex& lock, std::vector<uint8_t>& data,
                            volatile bool& hasDataFlag) {
