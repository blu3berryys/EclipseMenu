...
class ProjectionDelegate : public cocos2d::CCDirectorDelegate {
  virtual void updateProjection() override {
    ...
    kmMat4OrthographicProjection(&orthoMatrix, 0, size.width, size.height, 0,
                                 -1024, 1024);
...
void Recorder::start() {
  m_currentFrame.resize(
      m_renderSettings.m_width * m_renderSettings.m_height * 4, 0);
  ...
  utils::get<cocos2d::CCDirector>()->m_pProjectionDelegate =
      new ProjectionDelegate();
...
void Recorder::stop() {
  ...
  utils::get<cocos2d::CCDirector>()->setProjection(
      cocos2d::ccDirectorProjection::kCCDirectorProjection2D);
}
...
void Recorder::stopAudio() {
  ...
  std::filesystem::path tempPath =
      m_renderSettings.m_outputFile.parent_path() / "music.mp4";

  (void)ffmpeg::AudioMixer::mixVideoRaw(m_renderSettings.m_outputFile, data,
                                        tempPath);
