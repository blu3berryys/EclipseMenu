  ...
  if (val.is##type()) {                                                        \
    ...
    e->setResult(Err("Value is not a " #type));                                \
        ...
        if constexpr (std::same_as<T, std::string>) {
          ...
          if (val.isNull())
            e->setResult(Ok(std::monostate{}));
          else
            e->setResult(Err("Value is not null"));
        } else {
  ...
  new EventListener<EventFilter<events::SetRiftVariableEvent<T>>>(
      +[](events::SetRiftVariableEvent<T> *e) {
        if constexpr (std::same_as<T, label::null_t>) {
          labels::VariableManager::get().removeVariable(
              std::string(e->getName()));
        } else {
          labels::VariableManager::get().setVariable(
              std::string(e->getName()), rift::Value::from<T>(e->getValue()));
        }
        return ListenerResult::Stop;
      });
}
...
$execute {
  new EventListener<EventFilter<events::CreateMenuTabEvent>>(
      +[](events::CreateMenuTabEvent *e) {
        gui::MenuTab::find(e->getName());
        return ListenerResult::Stop;
      });

  /* Components */
  new EventListener<EventFilter<events::AddLabelEvent>>(
      +[](events::AddLabelEvent *e) {
        auto tab = gui::MenuTab::find(e->getTabName());
        auto label = tab->addLabel(e->getTitle());
        e->setUniqueID(label->getUID());
        return ListenerResult::Stop;
      });
  new EventListener<EventFilter<events::AddToggleEvent>>(
      +[](events::AddToggleEvent *e) {
        auto tab = gui::MenuTab::find(e->getTabName());
        auto toggle = tab->addToggle(e->getTitle(), e->getID());
        toggle
            ->callback([callback = std::get<0>(e->getCallbacks())](bool value) {
  ...
  new EventListener<EventFilter<events::AddButtonEvent>>(
      +[](events::AddButtonEvent *e) {
        auto tab = gui::MenuTab::find(e->getTabName());
  ...
  /* RIFT */
  new EventListener<EventFilter<events::FormatRiftStringEvent>>(
      +[](events::FormatRiftStringEvent *e) {
        auto res = rift::format(e->getSource(),
                                labels::VariableManager::get().getVariables());
        if (res.isErr()) {
