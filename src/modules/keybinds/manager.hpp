...
class Manager {
 public:
  ...
  /// @brief Register a keybind to the manager (in case the keybind will be
  /// later used)
  /// @param id The ID of the keybind.
  /// @param title The title of the keybind.
  /// @param callback The callback to execute when the keybind is pressed.
  Keybind& registerKeybind(const std::string& id, const std::string& title,
                           const std::function<void(bool)>& callback);

  /// @brief Register a keybind without adding it to the keybinds UI tab. Useful
  /// for internal keybinds.
  /// @param id The ID of the keybind.
  /// @param callback The callback to execute when the keybind is pressed.
  Keybind& addListener(const std::string& id,
                       const std::function<void(bool)>& callback);

  /// @brief Unregister a keybind from the manager. This will completely remove
  ...
  /// @return All keybinds.
  [[nodiscard]] const std::vector<Keybind>& getKeybinds() const {
    return m_keybinds;
  }
 ...
 private:
  ...
  Keybind& registerKeybindInternal(const std::string& id,
