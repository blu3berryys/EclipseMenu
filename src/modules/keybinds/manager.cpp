...
#define CASE(key, name)                                                        \
  ...
  switch (key) {
    ...
    CASE(NumPad8, "Num 8");
    CASE(NumPad9, "Num 9");
    CASE(NumPadDecimal, "Num .");
    CASE(NumPadDivide, "Num /");
    CASE(NumPadMultiply, "Num *");
    CASE(NumPadSubtract, "Num -");
    CASE(NumPadAdd, "Num +");
    CASE(NumPadEnter, "Num Enter");
    CASE(NumPadEqual, "Num =");
    ...
    CASE(LeftAlt, "Alt");
    CASE(LeftSuper, "Super");
    CASE(RightShift, "RShift");
    CASE(RightControl, "RCtrl");
    CASE(RightAlt, "RAlt");
    CASE(RightSuper, "RSuper");

    CASE(MouseLeft, "LMB");
    CASE(MouseRight, "RMB");
    CASE(MouseMiddle, "MMB");
    CASE(MouseButton4, "Mouse 4");
    CASE(MouseButton5, "Mouse 5");

    CASE(MenuKey, "Menu Key");
    CASE(LastKey, "Last Key");
  default:
    return "Unknown";
  }
#undef CASE
...
class $modify(KeybindingsManagerCCEGLVHook,
              cocos2d::CCEGLView){static void onModify(auto &self){
    FIRST_PRIORITY("cocos2d::CCEGLView::onGLFWKeyCallback");
}

void onGLFWKeyCallback(GLFWwindow *window, int key, int scancode, int action,
                       int mods) {
...
class $modify(KeybindingsManagerCCKDHook,
              cocos2d::CCKeyboardDispatcher){static void onModify(auto &self){
...
static std::map<std::string, std::shared_ptr<gui::KeybindComponent>>
    s_keybindComponents;
...
void updateHintLabel() {
  ...
  s_hintLabel->setText(components.size() <= 3 ? i18n::get_("keybinds.hint")
                                              : "");
...
Keybind &Manager::registerKeybindInternal(
    ...
    const std::function<void(bool)> &callback, bool internal) {
...
Keybind &Manager::registerKeybind(const std::string &id,
                                  const std::string &title,
                                  const std::function<void(bool)> &callback) {
  return this->registerKeybindInternal(id, title, callback, false);
}

Keybind &Manager::addListener(const std::string &id,
                              const std::function<void(bool)> &callback) {
    ...
    if (keybind.getId() == id) {
      ...
      gui::Engine::queueAfterDrawing([idStr, state, keybind] {
        ...
        if (state) {
          ...
          if (auto keybindComponent = s_keybindComponents[idStr];
              keybindComponent) {
            tab->removeComponent(keybindComponent);
          }
        }
...
void Manager::registerKeyPress(Keys key) {
  ...
  bool ignoreInPlayLayer = !utils::get<PlayLayer>() &&
                           config::get<bool>("keybind.in-game-only", false);

  for (auto &keybind : m_keybinds) {
    if (keybind.getKey() == key &&
        (keybind.isInitialized() || keybind.isInternal())) {
...
std::optional<std::reference_wrapper<Keybind>>
Manager::getKeybind(std::string_view id) {
  for (auto &keybind : m_keybinds) {
    if (keybind.getId() == id)
      return keybind;
  }
...
void Manager::setupTab() {
  ...
  tab->addKeybind("keybinds.open-menu", "menu.toggleKey")
      ->callback([](Keys key) {
        ...
        if (auto keybind = Manager::get()->getKeybind("menu.toggle");
            keybind.has_value())
          keybind->get().setKey(key);
      });

  tab->addToggle("keybinds.in-game-only")->setDescription();
