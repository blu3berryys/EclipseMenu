...
void Bot::setState(State state) { m_state = state; }

void Bot::clearInputs() { m_replay.inputs.clear(); }

void Bot::restart() { m_inputIndex = 0; }

void Bot::removeInputsAfter(int frame) {
  std::erase_if(m_replay.inputs, [&](const gdr::Input& input) -> bool {
    return input.frame > frame;
  });
}

void Bot::recordInput(int frame, PlayerButton button, bool player2,
                      bool pressed) {
  m_replay.inputs.emplace_back(frame, static_cast<int>(button), player2,
                               pressed);
}
...
void Bot::setLevelInfo(gdr::Level levelInfo) { m_replay.levelInfo = levelInfo; }

void Bot::save(std::filesystem::path path) {
  m_replay.author = utils::get<GJAccountManager>()->m_username;
  m_replay.duration = m_replay.inputs.size() > 0
                          ? m_replay.inputs[m_replay.inputs.size() - 1].frame /
                                m_replay.framerate
                          : 0;
