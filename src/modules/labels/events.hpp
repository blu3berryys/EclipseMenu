...
inline static std::array eventNames = {"Always", "Custom", "On Death",
                                       "On Button Hold", "On Noclip Death"};

inline static std::array visibleNames = {"Don't Modify", "Show", "Hide"};
...
struct LabelEvent {
  ...
  float duration = 0.f; // how long the new state should last after condition is
                        // no longer met
  float easing =
      0.f; // time to ease in/out the animation (only for scale and color)
};
...
struct Event {
  ...
  Event(size_t labelId, size_t eventId, LabelEvent *event,
