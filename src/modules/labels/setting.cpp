...
Event *handleEvent(const LabelEvent &event, const LabelSettings *label) {
  ...
  if (shouldProcess)
    manager.queueEvent(event, const_cast<LabelSettings *>(label));
  else
    manager.dequeueEvent(event, label);
  ...
  s_listener.bind([this](FileEvent::Event *event) {
    if (auto value = event->getValue()) {
      ...
      file << nlohmann::json(*this).dump(
          4, ' ', false, nlohmann::detail::error_handler_t::ignore);
      file.close();
    }
  });

  s_listener.setFilter(
      geode::utils::file::pick(geode::utils::file::PickMode::SaveFile,
...
void to_json(nlohmann::json &json, const LabelEvent &event) {
  json = nlohmann::json{
      {"enabled", event.enabled},     {"type", static_cast<int>(event.type)},
      {"condition", event.condition}, {"delay", event.delay},
