...
$on_mod(Loaded) { rift::Config::get().makeFunction("cfg", getConfigValue); }
...
void VariableManager::init() {
  ...
  m_variables["geodeVersion"] =
      geode::Loader::get()->getVersion().toNonVString();
  m_variables["platform"] = GEODE_WINDOWS("Windows") GEODE_ANDROID("Android")
      GEODE_MACOS("macOS") GEODE_IOS("iOS");
  ...
  m_variables["enabledMods"] = rift::Value::integer(std::ranges::count_if(
      allMods, [](auto *mod) { return mod->shouldLoad(); }));
...
void VariableManager::setVariable(const std::string &name,
                                  const rift::Value &value) {
...
void VariableManager::fetchLevelData(GJGameLevel *level) {
  if (!level) {
    ...
    constexpr std::array keys = {
        ...
        "isRobtopLevel", "levelAttempts",   "levelStars",       "difficulty",
        "difficultyKey", "practicePercent", "bestPercent",      "bestTime",
        "best"};
  ...
  bool isRobtopLevel =
      (levelID > 0 && levelID < 100) || (levelID >= 3001 && levelID <= 6000);
  ...
  m_variables["author"] =
      rift::Value::string(isRobtopLevel ? "RobTop" : level->m_creatorName);
...
void VariableManager::fetchGameplayData(GJBaseGameLayer *gameLayer) {
  if (!gameLayer) {
    ...
    constexpr std::array keys = {
        ...
        "levelTime",      "levelLength", "levelDuration",  "time",
        "frame",          "isDead",      "isDualMode",     "noclipDeaths",
        "noclipAccuracy", "progress",    "editorMode",     "realProgress",
        "objects"};
    for (const auto &key : keys) {
      removeVariable(key);
    }
  ...
  m_variables["frameReal"] =
      gameLayer->m_gameState.m_levelTime * utils::getTPS();
  ...
  for (int i = 0; i < s_coins.size(); ++i) {
    ...
    auto pickedUp = gameLayer->m_collectedItems->objectForKey(
                        std::to_string(i + 1)) != nullptr;
...
class $modify(VariablesPLHook, PlayLayer) {
  ...
  void saveBestRun() {
    ...
    if ((fields->m_bestRun - fields->m_runFrom) >=
        (fields->m_lastBestRun - fields->m_lastRunFrom)) {
