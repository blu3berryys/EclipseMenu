...
geode::Result<std::string> Shader::compile(
  ...
  if (!vertexSource)
    return geode::Err("failed to read vertex shader at path {}: {}",
                      vertexPath.string(), vertexSource.unwrapErr());

  auto fragmentSource = geode::utils::file::readString(fragmentPath);
  if (!fragmentSource)
    return geode::Err("failed to read fragment shader at path {}: {}",
  ...
  return geode::Ok(
      fmt::format("shader compilation successful. logs:\nvert:\n{}\nfrag:\n{}",
...
void RenderTexture::setup(GLsizei width, GLsizei height) {
  ...
  if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
    geode::log::error(
        "pp fbo not complete, uh oh! i guess i will have to cut off ur pp now");
...
void setupPostProcess() {
  ...
  GLfloat ppVertices[] = {// positions   // texCoords
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(ppVertices), &ppVertices,
               GL_STATIC_DRAW);
  ...
  auto vertexPath =
      std::string{cocos2d::CCFileUtils::get()->fullPathForFilename(
          "pp-vert.glsl"_spr, false)};
  auto fragmentPath =
      std::string{cocos2d::CCFileUtils::get()->fullPathForFilename(
          "pp-frag.glsl"_spr, false)};

  auto res = ppShader.compile(vertexPath, fragmentPath);
  if (!res)
    return geode::log::error("Failed to compile shader: {}", res.unwrapErr());
  // log::info("{}", res.unwrap());
  ...
  if (!res2)
    return geode::log::error("Failed to link shader: {}", res2.unwrapErr());
  // log::info("{}", res.unwrap());

  cocos2d::ccGLUseProgram(ppShader.program);
  glUniform1i(glGetUniformLocation(ppShader.program, "screen"), 0);
  glUniform2f(glGetUniformLocation(ppShader.program, "screenSize"), size.width,
              size.height);
...
class $modify(BlurCCNHook, cocos2d::CCNode){void visit() override{
...
class $modify(BlurGMHook,
              GameManager){void reloadAllStep5(){GameManager::reloadAllStep5();
cleanupPostProcess();
setupPostProcess();
}
}
...
void update(float) {
  ...
  if (duration == 0.f)
    blurProgress = toggled ? 1.f : 0.f;
  else
    blurProgress = blurTimer / duration;
}
}
