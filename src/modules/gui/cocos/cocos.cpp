...
void CocosRenderer::shutdown(bool noCleanup) {
  for (auto popup : m_optionsPopups) popup->removeFromParentAndCleanup(true);
  m_optionsPopups.clear();

  for (auto modal : m_modals) modal->removeFromParentAndCleanup(true);
  m_modals.clear();

  if (!m_popup) return;

  if (!noCleanup) m_popup->removeFromParentAndCleanup(true);
  m_popup = nullptr;
  utils::updateCursorState(isToggled());
}
