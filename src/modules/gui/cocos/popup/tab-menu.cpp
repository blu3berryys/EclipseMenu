...
TabMenu *TabMenu::create(Tabs const &tabs,
...
TabButton *TabButton::create(const std::string &name,
...
bool TabMenu::init(Tabs const &tabs, std::function<void(int)> const &callback) {
  ...
  for (auto const &tab : tabs) {
    ...
    auto tabButton = geode::cocos::CCMenuItemExt::createSpriteExtra(
        // ButtonSprite::create(tabName.c_str(), width, true, "bigFont.fnt",
        // "GJ_button_01.png", height, 0.5f),
        tabSpr, [this, callback](auto caller) {
...
void TabButton::setState(bool active) {
  ...
  if (m_bgSprite == nullptr || m_label == nullptr)
    return;
  const auto tm = ThemeManager::get();
  auto colorLbl = (!active) ? tm->getButtonForegroundColor()
                            : tm->getButtonActivatedForeground();
  auto colorBG = (!active) ? tm->getButtonBackgroundColor()
