...
bool ScrollLayer::ccTouchBegan(cocos2d::CCTouch *touch,
                               cocos2d::CCEvent *event) {
  if (geode::cocos::nodeIsVisible(this)) {
    bool value = CCScrollLayerExt::ccTouchBegan(touch, event);
    if (value) {
      m_touchStart = touch;
      auto touchPos = utils::get<cocos2d::CCDirector>()->convertToGL(
          m_touchStart->getLocationInView());
...
void ScrollLayer::ccTouchEnded(cocos2d::CCTouch *touch,
                               cocos2d::CCEvent *event) {
...
void ScrollLayer::ccTouchMoved(cocos2d::CCTouch *touch,
                               cocos2d::CCEvent *event) {
  CCScrollLayerExt::ccTouchMoved(touch, event);
  m_touchMoved = true;
  auto touchPoint = utils::get<cocos2d::CCDirector>()->convertToGL(
...
void ScrollLayer::claimTouch(cocos2d::CCTouch *touch) {
  auto touchDispatcher =
      utils::get<cocos2d::CCDirector>()->getTouchDispatcher();
  auto handler = static_cast<cocos2d::CCTargetedTouchHandler *>(
      touchDispatcher->findHandler(this));
  if (handler) {
...
void ScrollLayer::cancelAndStoleTouch(cocos2d::CCTouch *touch,
                                      cocos2d::CCEvent *event) {
...
ScrollLayer::ScrollLayer(cocos2d::CCRect const &rect, bool scrollWheelEnabled,
                         bool vertical)
  ...
  m_contentLayer =
      geode::GenericContentLayer::create(rect.size.width, rect.size.height);
...
void ScrollLayer::visit() {
  if (m_cutContent && this->isVisible()) {
    ...
    if (this->getParent()) {
      ...
      utils::get<cocos2d::CCEGLView>()->setScissorInPoints(
          bottomLeft.x, bottomLeft.y, size.width, size.height);
    }
  }
...
void ScrollLayer::scrollToTop() {
  auto listTopScrollPos =
      -m_contentLayer->getContentHeight() + this->getContentHeight();
  m_contentLayer->setPositionY(listTopScrollPos);
}
ScrollLayer *ScrollLayer::create(cocos2d::CCRect const &rect, bool scroll,
                                 bool vertical) {
  auto ret = new ScrollLayer(rect, scroll, vertical);
  ret->autorelease();
  return ret;
}
ScrollLayer *ScrollLayer::create(cocos2d::CCSize const &size, bool scroll,
                                 bool vertical) {
...
void ScrollLayer::fixTouchPrio() {
  ...
  if (auto delegate = geode::cast::typeinfo_cast<CCTouchDelegate *>(this)) {
    if (auto handler =
            cocos2d::CCTouchDispatcher::get()->findHandler(delegate)) {
      geode::Loader::get()->queueInMainThread([this, handler, delegate,
                                               oldThis]() {
