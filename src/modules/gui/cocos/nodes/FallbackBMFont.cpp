...
void FallbackBMFont::Label::createFontChars() {
  ...
  nextFontPositionY = -(m_pConfiguration->m_nCommonHeight -
                        m_pConfiguration->m_nCommonHeight * quantityOfLines);
  ...
  for (uint32_t i = 0; i < stringLen; ++i) {
    ...
    kerningAmount = this->kerningAmountForFirst(
        prev, c,
        usingFallback ? m_parent->m_fallbackConfiguration->m_pKerningDictionary
                      : m_pConfiguration->m_pKerningDictionary);
    cocos2d::tCCFontDefHashElement *element = nullptr;

    uint32_t key = c;
    cocos2d::tCCFontDefHashElement *root =
        usingFallback ? m_parent->m_fallbackConfiguration->m_pFontDefDictionary
                      : m_pConfiguration->m_pFontDefDictionary;
    ...
    rect = {fontDef.rect.origin.x / scaleFactor,
            fontDef.rect.origin.y / scaleFactor,
            fontDef.rect.size.width / scaleFactor,
            fontDef.rect.size.height / scaleFactor};
    ...
    if (usingFallback) {
      fontChar = static_cast<cocos2d::CCSprite *>(
          m_spriteBatch->getChildByTag(fallbackIndex));
    } else {
      fontChar =
          static_cast<cocos2d::CCSprite *>(this->getChildByTag(normalIndex));
    }
    ...
    if (!fontChar) {
      ...
      fontChar->initWithTexture(usingFallback ? m_spriteBatch->getTexture()
                                              : m_pobTextureAtlas->getTexture(),
                                rect);
      ...
      // Color MUST be set before opacity, since opacity might change color if
      // OpacityModifyRGB is on
    ...
    cocos2d::CCPoint fontPos{static_cast<float>(nextFontPositionX) +
                                 fontDef.xOffset +
                                 fontDef.rect.size.width * 0.5f + kerningAmount,
                             static_cast<float>(nextFontPositionY) + yOffset -
                                 rect.size.height * 0.5f * scaleFactor};
    fontChar->setPosition({fontPos.x / scaleFactor, fontPos.y / scaleFactor});
  ...
  this->setContentSize(
      {tmpSize.width / scaleFactor, tmpSize.height / scaleFactor});
}
...
bool FallbackBMFont::init(const std::string &text, const std::string &font,
  ...
  m_fallbackBatch = cocos2d::CCSpriteBatchNode::create(
      m_fallbackConfiguration->getAtlasName());
...
static uint64_t getEmoji(const std::u16string &str, uint32_t &index,
                         uint32_t len) {
...
void EmojiLabel::Label::createFontChars() {
  ...
  for (uint32_t i = 0; i < stringLen; ++i) {
    ...
    if (!charset->contains(c)) {
      ...
      if (emoji != 0) {
        ...
        if (it != g_emojis.end()) {
          ...
          auto sprite = static_cast<cocos2d::CCSprite *>(
              m_spriteBatch->getChildByTag(emojiIndex));
          if (!sprite) {
            // create sprite
            ...
            // update sprite frame
            auto spriteFrame =
                cocos2d::CCSpriteFrameCache::get()->spriteFrameByName(
                    frame.data());
          ...
          auto sizeInPixels = cocos2d::CCSize{size.width * scaleFactor,
          ...
          sprite->setPosition(
              {(nextFontPositionX + sizeInPixels.width / 2) / scaleFactor,
               (nextFontPositionY + m_pConfiguration->m_nCommonHeight / 2) /
                   scaleFactor});
    ...
    // See issue 1343. cast( signed short + unsigned integer ) == unsigned
    // integer (sign is lost!)
    int yOffset = m_pConfiguration->m_nCommonHeight - fontDef.yOffset;
    cocos2d::CCPoint fontPos{static_cast<float>(nextFontPositionX) +
                                 fontDef.xOffset +
                                 fontDef.rect.size.width * 0.5f + kerningAmount,
...
EmojiLabel *EmojiLabel::create(const std::string &text,
                               const std::string &font) {
