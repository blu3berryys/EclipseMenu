...
inline static std::u16string UTF8ToUTF16(const std::string &utf8) {
  ...
  MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), -1,
                      reinterpret_cast<wchar_t *>(utf16.data()), len);
...
inline static void setBatchSpriteColor(cocos2d::CCSpriteBatchNode *batch,
                                       const cocos2d::ccColor3B &color) {
  if (auto children = batch->getChildren()) {
    for (int i = 0; i < children->count(); i++) {
      static_cast<cocos2d::CCSprite *>(children->objectAtIndex(i))
          ->setColor(color);
    }
  }
}

inline static void setBatchSpriteOpacity(cocos2d::CCSpriteBatchNode *batch,
                                         GLubyte opacity) {
  if (auto children = batch->getChildren()) {
    for (int i = 0; i < children->count(); i++) {
      static_cast<cocos2d::CCSprite *>(children->objectAtIndex(i))
...
class BaseLabel : public cocos2d::CCLabelBMFont {
 public:
  ...
  void setString(const char *newString, bool needUpdateLabel) override {
    ...
    this->BaseLabel::setString(reinterpret_cast<uint16_t *>(m_string.data()),
                               needUpdateLabel);
  }
  ...
  void setFntFile(std::string_view fntFile) {
    ...
    this->setTexture(utils::get<cocos2d::CCTextureCache>()->addImage(
        m_pConfiguration->getAtlasName(), false));
    static_cast<T *>(this)->T::createFontChars();
  }

  int kerningAmountForFirst(
      uint16_t first, uint16_t second,
      const cocos2d::tCCKerningHashElement *dictionary) const {
    int ret = 0;
  ...
  int kerningAmountForFirst(uint16_t first, uint16_t second) const {
    return kerningAmountForFirst(first, second,
...
/// @brief Label node that supports two bitmap fonts, using the second one as a
/// fallback. This is useful for languages that have characters not supported by
...
class FallbackBMFont : public cocos2d::CCNode {
 ...
 public:
  ...
  ~FallbackBMFont() override { m_fallbackConfiguration->release(); }
