...
class ColorComponentNode
 ...
 public:
  bool init(float width) override {
    ...
    m_colorBtn = geode::cocos::CCMenuItemExt::createSpriteExtra(
        m_colorSprite, [this](auto) {
          geode::ColorPickPopup* popup;
          if (m_component->hasOpacity()) {
            popup = geode::ColorPickPopup::create(
                static_cast<cocos2d::ccColor4B>(m_component->getValue()));
          } else {
            popup = geode::ColorPickPopup::create(
                m_component->getValue().toCCColor3B());
          }
