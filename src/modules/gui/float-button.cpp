...
void FloatingButton::setScale(float scale) { m_sprite->setScale(scale); }
...
bool FloatingButton::init() {
  ...
  m_sprite->setPosition({config::get<float>("float-btn.x", 480.f),
...
void FloatingButton::fadeOut() const {
  m_sprite->stopAllActions();
  m_sprite->runAction(
      CCSequence::create(CCDelayTime::create(1.f),
                         CCFadeTo::create(0.2f, m_minOpacity * 255), nullptr));
}
...
void FloatingButton::ccTouchEnded(CCTouch* touch, CCEvent* event) {
  ...
  if (m_callback)
    ...
    geode::log::warn("FloatingButton callback unspecified!");
}

void FloatingButton::ccTouchMoved(CCTouch* touch, CCEvent* event) {
  auto touchPos = this->convertToNodeSpace(touch->getLocation());
  if (!m_shouldMove &&
      ccpDistance(touchPos, m_holdPosition) < MIN_MOVE_DISTANCE)
    return;
...
class $modify(CCScene){/// Allows our button to stay top-most, passing z-order
                       /// of the node below the button.
                       /// Shout-out to QOLMod for the idea.
                       int getHighestChildZ(){auto btn = FloatingButton::get();
