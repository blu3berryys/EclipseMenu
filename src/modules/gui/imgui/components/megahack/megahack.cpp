...
bool Megahack::checkbox(const std::string &label, bool &value,
                        bool isSearchedFor,
                        const std::function<void()> &postDraw) const {
  auto tm = ThemeManager::get();
  auto textColor = value ? tm->getCheckboxForegroundColor()
                         : tm->getButtonDisabledForeground();
  ...
  ImGui::PushStyleColor(
      ImGuiCol_Text,
      static_cast<ImVec4>(isSearchedFor ? tm->getSearchedColor() : textColor));
  ...
  ImGui::GetWindowDrawList()->AddRectFilled(
      ImVec2(ImGui::GetItemRectMax().x - 5 * scale,
             ImGui::GetItemRectMin().y + 1 * scale),
      ImVec2(ImGui::GetItemRectMax().x - 2 * scale,
             ImGui::GetItemRectMax().y - 1 * scale),
      textColor);
...
bool Megahack::checkboxWithSettings(const std::string &label, bool &value,
  ...
  ImGui::PopStyleVar(2);
  bool openPopup =
      ImGui::Button(fmt::format("##open_{}", label).c_str(), arrowSize);
  ImGui::PopItemWidth();
  ImGui::PopStyleColor(4);
  ...
  ImGui::GetWindowDrawList()->AddTriangleFilled(
      ImVec2(right, top), ImVec2(left, bottom), ImVec2(right, bottom),
      triangleColor);
...
bool Megahack::button(const std::string &text, bool isSearchedFor) const {
  ...
  Color color;
  if (isMouseOver) {
    color = isItemActive ? tm->getButtonActivatedBackground()
                         : tm->getButtonHoveredBackground();
  } else {
