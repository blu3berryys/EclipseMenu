...
bool MegaOverlay::checkbox(const std::string& label, bool& value,
                           bool isSearchedFor,
                           const std::function<void()>& postDraw) const {
  auto tm = ThemeManager::get();
  auto textColor =
      value ? tm->getCheckboxForegroundColor() : tm->getDisabledColor();
  auto checkboxColor = value ? tm->getCheckboxCheckmarkColor()
                             : tm->getDisabledColor().darken(0.2f);
  ...
  ItemSize({check_bb.GetWidth(), check_bb.GetHeight() - 3},
  ...
  if (label_size.x > 0) {
    ItemSize({check_bb.GetWidth(), check_bb.GetHeight() - 3},
             style.FramePadding.y);
    total_bb = ImRect(check_bb.Min,
                      ImVec2(text_bb.Max.x - cc_pad * 2.0f * io.FontGlobalScale,
                             check_bb.Max.y));
  }
  ...
  window->DrawList->AddRectFilled(
      ImVec2(check_bb.Min.x, check_bb.Min.y),
      ImVec2(check_bb.Min.x + cc_sz, check_bb.Max.y), checkboxColor,
      style.FrameRounding);
  ...
  if (label_size.x > 0.0f) {
    PushStyleColor(ImGuiCol_Text,
                   static_cast<ImVec4>(isSearchedFor ? tm->getSearchedColor()
                                                     : textColor));
    RenderText(ImVec2(check_bb.Min.x + cc_sz + cc_pad, text_pos.y),
               label.c_str());
    PopStyleColor();
  }
