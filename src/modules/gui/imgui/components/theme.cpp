...
truncateString(std::string_view str, float availWidth, bool canDelete = false) {
  ...
  if (labelSize.x > labelMaxWidth) {
    ...
    return {fmt::format("{}...", str.substr(0, labelEnd)), labelMaxWidth};
  }
...
void Theme::handleTooltip(const std::string &text) {
  ...
  if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text,
                          static_cast<ImVec4>(tm->getForegroundColor()));
...
void Theme::handleKeybindMenu(std::string_view id) {
  ...
  if (ImGui::IsItemClicked(1) ||
      (ImGui::IsItemClicked(0) && ImGui::GetIO().KeyShift)) {
  ...
  if (ImGui::BeginPopup(popupId.c_str())) {
    ...
    if (keybind.has_value()) {
      ...
      if (!keybindRef.isInitialized() &&
          ImGui::MenuItem(i18n::get("keybinds.add").data())) {
        keybinds->setKeybindState(id, true);
      } else if (keybindRef.isInitialized() &&
                 ImGui::MenuItem(i18n::get("keybinds.remove").data())) {
...
void Theme::update() {
  ...
  style.ItemSpacing =
      ImVec2(tm->getHorizontalSpacing(), tm->getVerticalSpacing());
  style.ItemInnerSpacing =
      ImVec2(tm->getHorizontalInnerSpacing(), tm->getVerticalInnerSpacing());
...
bool Theme::beginWindow(const std::string &title) {
  ...
  auto tm = ThemeManager::get();
  ImGui::PushStyleColor(ImGuiCol_Text,
                        static_cast<ImVec4>(tm->getTitleForegroundColor()));
...
void Theme::visitLabel(const std::shared_ptr<LabelComponent> &label) const {
  ...
  if (label->isSearchedFor())
    ImGui::PushStyleColor(
        ImGuiCol_Text,
        static_cast<ImVec4>(ThemeManager::get()->getSearchedColor()));
  else
    ImGui::PushStyleColor(
        ImGuiCol_Text,
        static_cast<ImVec4>(ThemeManager::get()->getForegroundColor()));
...
void Theme::visitRadioButton(
  ...
  ImGui::PushStyleColor(
      ImGuiCol_CheckMark,
      static_cast<ImVec4>(ThemeManager::get()->getCheckboxCheckmarkColor()));
  if (ImGui::RadioButton(i18n::get(radio->getTitle()).data(), &value,
                         radio->getChoice())) {
...
void Theme::visitFilesystemCombo(
  ...
  ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x *
                       (title.empty() ? .9f : .5f));
  if (ImGui::BeginCombo(
          fmt::format("##{}", title).c_str(),
          value.empty() ? "None" : value.filename().stem().string().c_str())) {
...
void Theme::visitSlider(const std::shared_ptr<SliderComponent> &slider) const {
  ...
  if (ImGui::SliderFloat(i18n::get(slider->getTitle()).data(), &value,
                         slider->getMin(), slider->getMax(),
                         slider->getFormat().c_str())) {
...
void Theme::visitInputFloat(
  ...
  ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x * 0.35f);
  if (ImGui::InputFloat(fmt::format("##{}", title).c_str(), &value, 0, 0,
                        inputFloat->getFormat().c_str())) {
...
void Theme::visitColor(const std::shared_ptr<ColorComponent> &color) const {
  ...
  if (color->hasOpacity()) {
    changed = ImGui::ColorEdit4(fmt::format("##{}", title).c_str(),
                                value.data(), ImGuiColorEditFlags_NoInputs);
  } else {
    changed = ImGui::ColorEdit3(fmt::format("##{}", title).c_str(),
                                value.data(), ImGuiColorEditFlags_NoInputs);
  }
...
void Theme::visitKeybind(
  ...
  const auto &[truncatedLabel, maxWidth] =
      truncateString(title, availWidth, canDelete);
  ...
  ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                        ImVec4(0.07f, 0.07f, 0.07f, 0.5f));
  ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                        ImVec4(0.04f, 0.04f, 0.04f, 0.5f));

  auto key =
      config::get<keybinds::Keys>(keybind->getId(), keybinds::Keys::None);
...
void Theme::visitLabelSettings(
    const std::shared_ptr<LabelSettingsComponent> &labelSettings) const {
  auto *settings = labelSettings->getSettings();

  if (this->checkboxWithSettings(
          settings->name, settings->visible, labelSettings->isSearchedFor(),
          [this, settings, labelSettings] {
            ...
            if (ImGui::Combo(i18n::get("labels.font").data(), &currentFont,
                             labels::fontNames.data(),
                             labels::fontNames.size())) {
              settings->font = labels::fontFiles[currentFont];
              labelSettings->triggerEditCallback();
            }

            auto labelScale = settings->scale;
            if (ImGui::InputFloat(i18n::get("labels.scale").data(), &labelScale,
                                  0.f, 0.f, "%.2f")) {
            ...
            if (ImGui::ColorEdit3(i18n::get("labels.color").data(),
                                  color.data(), ImGuiColorEditFlags_NoInputs)) {
              settings->color = color;
              labelSettings->triggerEditCallback();
            }

            auto opacity = settings->color.a;
            if (ImGui::InputFloat(i18n::get("labels.opacity").data(), &opacity,
                                  0.0f, 1.0f)) {
            ...
            if (ImGui::Combo(i18n::get("labels.alignment").data(),
                             reinterpret_cast<int *>(&alignment),
                             labels::alignmentNames.data(),
                             labels::alignmentNames.size())) {
              settings->alignment = alignment;
              labelSettings->triggerEditCallback();
            }

            auto absolute = settings->absolutePosition;
            if (this->checkbox(i18n::get("labels.absolute").data(), absolute,
                               false)) {
            ...
            if (absolute) {
              ...
              if (ImGui::InputFloat2(i18n::get("labels.offset").data(),
            ...
            if (ImGui::CollapsingHeader(i18n::get("labels.events").data())) {
              ...
              for (auto &event : events) {
                ImGui::PushID(&event);
                {
                  ...
                  changed |= this->checkbox(i18n::get_("labels.events.enabled"),
                                            event.enabled, false);
                  changed |= ImGui::Combo(
                      i18n::get("labels.events.type").data(),
                      reinterpret_cast<int *>(&event.type),
                      labels::eventNames.data(), labels::eventNames.size());

                  if (event.type == labels::LabelEvent::Type::Custom) {
                    changed |= ImGui::InputText(
                        i18n::get("labels.events.condition").data(),
                        &event.condition);
                  }

                  {
                    int currentVisibleState =
                        event.visible.has_value()
                            ? (event.visible.value() ? 1 : 2)
                            : 0;
                    changed |= ImGui::Combo(
                        i18n::get("labels.events.visible").data(),
                        &currentVisibleState, labels::visibleNames.data(),
                        labels::visibleNames.size());
                    if (currentVisibleState == 0)
                      event.visible.reset();
                    else
                      event.visible = currentVisibleState == 1;
                  }

                  {
                    bool hasScale = event.scale.has_value();
                    if (this->checkbox(i18n::get_("labels.events.scale"),
                                       hasScale, false)) {
                      if (hasScale)
                        event.scale = 1.0f;
                      else
                    ...
                    if (hasScale) {
                      changed |= ImGui::InputFloat(
                          i18n::get("labels.scale").data(),
                          &event.scale.value(), 0.f, 0.f, "%.2f");
                    }
                  }

                  {
                    bool hasColor = event.color.has_value();
                    if (this->checkbox(i18n::get_("labels.events.color"),
                                       hasColor, false)) {
                      if (hasColor)
                        event.color = {1.f, 1.f, 1.f};
                      else
                        event.color.reset();
                      changed = true;
                    }
                    if (hasColor) {
                      changed |= ImGui::ColorEdit3(
                          i18n::get("labels.color").data(), event.color->data(),
...

                  {
                    ...
                    if (this->checkbox(i18n::get_("labels.events.opacity"),
                                       hasOpacity, false)) {
                      if (hasOpacity)
                        event.opacity = 1.0f;
                      else
                    ...
                    if (hasOpacity) {
                      changed |=
                          ImGui::InputFloat(i18n::get("labels.opacity").data(),
                                            &event.opacity.value(), 0.f, 1.f);
                    }
                  }

                  {
                    bool hasFont = event.font.has_value();
                    if (this->checkbox(i18n::get_("labels.events.font"),
                                       hasFont, false)) {
                      if (hasFont)
                        event.font = "bigFont.fnt";
                      else
                        event.font.reset();
                      changed = true;
                    }
                    if (hasFont) {
                      int currentFont2 =
                          labels::getFontIndex(event.font.value());
                      if (ImGui::Combo(i18n::get("labels.font").data(),
                                       &currentFont2, labels::fontNames.data(),
                                       labels::fontNames.size())) {
                  ...
                  changed |=
                      ImGui::InputFloat(i18n::get("labels.events.delay").data(),
                                        &event.delay, 0.f, 0.f, "%.2f");
                  changed |= ImGui::InputFloat(
                      i18n::get("labels.events.duration").data(),
                      &event.duration, 0.f, 0.f, "%.2f");
                  changed |= ImGui::InputFloat(
                      i18n::get("labels.events.easing").data(), &event.easing,
                      0.f, 0.f, "%.2f");
...
bool Theme::checkbox(const std::string &label, bool &value, bool isSearchedFor,
                     const std::function<void()> &postDraw) const {
  auto tm = ThemeManager::get();

  ImGui::PushStyleColor(
      ImGuiCol_Text,
      static_cast<ImVec4>(isSearchedFor ? tm->getSearchedColor()
                                        : tm->getCheckboxForegroundColor()));
  ImGui::PushStyleColor(ImGuiCol_CheckMark,
                        static_cast<ImVec4>(tm->getCheckboxCheckmarkColor()));
  ImGui::PushStyleColor(ImGuiCol_FrameBg,
                        static_cast<ImVec4>(tm->getCheckboxBackgroundColor()));
...
bool Theme::checkboxWithSettings(const std::string &label, bool &value,
  ...
  bool openPopup = ImGui::ArrowButton(fmt::format("##open_{}", label).c_str(),
                                      ImGuiDir_Right);
  ImGui::PopItemWidth();
...
bool Theme::button(const std::string &text, bool isSearchedFor) const {
  ...
  if (isSearchedFor)
    ImGui::PushStyleColor(ImGuiCol_Text,
                          static_cast<ImVec4>(tm->getSearchedColor()));

  ImGui::PushStyleColor(ImGuiCol_Button,
                        static_cast<ImVec4>(tm->getButtonBackgroundColor()));
  ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                        static_cast<ImVec4>(tm->getButtonHoveredBackground()));
  ImGui::PushStyleColor(
      ImGuiCol_ButtonActive,
      static_cast<ImVec4>(tm->getButtonActivatedBackground()));

  bool pressed =
      ImGui::Button(text.c_str(), ImVec2(ImGui::GetContentRegionAvail().x, 0));
