...
void FontManager::FontMetadata::load() {
  ...
  m_font = ImGui::GetIO().Fonts->AddFontFromFileTTF(
      m_path.string().c_str(), fontSize, nullptr,
      getGlyphRange(i18n::getRequiredGlyphRanges()));
}
...
void FontManager::fetchFonts() { m_availableFonts = fetchAvailableFonts(); }
...
bool ImGuiRenderer::isToggled() const { return m_isOpened; }
...
void ImGuiRenderer::reload() const { ImGuiCocos::get().reload(); }

void ImGuiRenderer::queueAfterDrawing(const std::function<void()>& func) {
  m_runAfterDrawingQueue.push_back(func);
}

void ImGuiRenderer::showPopup(const Popup& popup) { m_popups.push_back(popup); }
...
void ImGuiRenderer::renderPopups() {
  ...
  m_popups.erase(std::ranges::remove_if(m_popups,
                                        [&](const Popup& p) {
                                          for (auto* r : toRemove) {
                                            if (r->getId() == p.getId())
                                              return true;
