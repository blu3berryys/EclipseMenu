...
constexpr std::array themes = {"Default", "MegaHack"};
...
void TabbedLayout::toggle(bool state) {
  ...
  auto easingType =
      config::get("menu.animationEasingType", animation::Easing::Quadratic);
  auto easingMode =
      config::get("menu.animationEasingMode", animation::EasingMode::EaseInOut);
...
std::map<Window *, ImVec2> TabbedLayout::getStackedPositions() {
  ...
  auto columns =
      static_cast<int>((screenSize.x - margin) / (windowWidth + margin));
  ...
  for (auto &window : m_windows) {
    ...
    if (std::ranges::find(builtInWindows, window.getTitle()) !=
        builtInWindows.end())
      continue;
    ...
    positions[&window] = ImVec2(
        static_cast<float>(windowColumn) * (windowWidth + margin) + margin,
        *min);
...
void TabbedLayout::stackWindows() {
  ...
  for (auto &[window, target] : getStackedPositions()) {
    ...
    if (window->getPosition().x == target.x &&
        window->getPosition().y == target.y)
      continue;
