...
void SidebarLayout::recalculateSize(bool first) {
  ...
  auto initialSize =
      ImVec2(first ? 165 * ThemeManager::get()->getGlobalScale() : 250, 700);
  ...
  ImGui::SetWindowPos(ImVec2(first ? 10 : (screenSize.x - scaledSize.x - 10),
                             screenSize.y / 2 - scaledSize.y / 2));
}
...
void SidebarLayout::draw() {
  ...
  for (int i = 0; i < currentTabs.size(); i++) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Button, m_selectedTab == i
                                               ? style->Colors[ImGuiCol_Button]
                                               : ImVec4(0, 0, 0, 0));
    ImGui::PushStyleColor(ImGuiCol_Text,
                          currentTabs[i]->isSearchedFor()
  ...
  ImGui::Dummy(ImVec2(0.0f, ImGui::GetContentRegionAvail().y - 80 * scale -
                                style->ItemSpacing.y));
  ...
  ImGui::BeginChild("modules-wrapper",
