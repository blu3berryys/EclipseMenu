...
#define REGISTER_HACK(hackClass) \
  $execute { eclipse::hack::Hack::registerHack<hackClass>(); }

constexpr int32_t SAFE_HOOK_PRIORITY = 0x500000;
constexpr int32_t FIRST_HOOK_PRIORITY = -0x500000;

#define SAFE_SET_PRIO(name, prio)                                \
  do {                                                           \
  ...
  do {                                                                   \
    ...
    geode::queueInMainThread([hooks] {                                   \
      ...
      config::addDelegate(id, [hooks] {                                  \
        auto value = config::get(id, false);                             \
        for (auto h : hooks) (void)(value ? h->enable() : h->disable()); \
      });                                                                \
    });                                                                  \
  } while (0)
...
#define ENABLE_SAFE_HOOKS(class, ...) \
  static void onModify(auto &self) { SAFE_HOOKS(class, __VA_ARGS__); }

// Creates an onModify method with hooks set to FIRST_PRIORITY
#define ENABLE_FIRST_HOOKS(class, ...) \
  static void onModify(auto &self) { FIRST_HOOKS(class, __VA_ARGS__); }
...
// Creates an onModify method with all hooks added to delegate listener and
// specific hooks added to SAFE_PRIORITY
...
class Hack {
 public:
  ...
  static void registerHack() {
    registerHack(std::make_shared<T>());
  }
  ...
  /// @brief Returns whether the hack should be considered cheating and is
  /// enabled.
