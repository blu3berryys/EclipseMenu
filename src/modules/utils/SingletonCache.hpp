...
template <typename T>
concept HasSharedState =
    requires { T::get(); } || requires { T::sharedDispatcher(); };
...
template <typename T> base_type<T> *getNested() {
  if constexpr (std::is_same_v<base_type<T>, EditorUI>) {
    ...
    static_assert(!std::is_same_v<base_type<T>, base_type<T>>,
                  "Unsupported nested singleton type");
  }
  return nullptr;
}
...
template <SupportedSingleton T> base_type<T> *get() {
  ...
  if constexpr (NestedInstance<type>) {
    ...
    constexpr bool isStatic =
        not_one_of_v<type, PlayLayer, LevelEditorLayer, GJBaseGameLayer,
                     MenuLayer, cocos2d::CCScene>;
