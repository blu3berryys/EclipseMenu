...
nlohmann::json g_translations = {{"language-code", "en"},
...
std::string get_(std::string_view key) { return std::string(get(key)); }
...
void refreshStaticArrays() {
  ...
  for (size_t i = 0; i < labels::eventNames.size(); ++i) {
    labels::eventNames[i] =
        get(fmt::format("labels.events.types.{}", i)).data();
  }

  for (size_t i = 0; i < labels::visibleNames.size(); ++i) {
    labels::visibleNames[i] =
        get(fmt::format("labels.events.visible.{}", i)).data();
  }
}
...
std::optional<LanguageMetadata> getLanguageName(
  ...
  auto fallback = json.contains("language-fallback")
                      ? json["language-fallback"].get<std::string>()
                      : "en";
  auto charset = json.contains("language-charset")
                     ? json["language-charset"].get<std::string>()
                     : "default";

  return LanguageMetadata{json["language-name"].get<std::string>(),
                          json["language-code"].get<std::string>(), fallback,
                          charset, path};
}
...
std::vector<LanguageMetadata> fetchAvailableLanguages() {
  ...
  auto globLangs = [&](std::filesystem::path const& path) {
    ...
    for (auto& entry : std::filesystem::directory_iterator(path)) {
      ...
      if (filename.size() < 10 ||
          filename.substr(filename.size() - 10) != ".lang.json")
        continue;
...
std::vector<std::string> getAvailableLanguages() {
  ...
  for (const auto& lang : langNames) {
    if (lang.code == currentLang) {
      langNamesVec.push_back(
          fmt::format("{} ({})", i18n::get("language-native"), lang.code));
      continue;
    }
