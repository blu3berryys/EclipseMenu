...
bool DownloadPopup::setup(std::string const& charset) {
  ...
  cocos2d::ccTexParams params = {GL_LINEAR, GL_LINEAR, GL_REPEAT,
                                 GL_CLAMP_TO_EDGE};
  ...
  m_mainLayer->addChildAtPosition(m_progressBarBG, geode::Anchor::Bottom,
                                  {0, 20});
...
void DownloadPopup::setProgress(float progress) {
  m_progressLabel->setString(fmt::format("{:.2f}% ({}/{})", progress * 100,
                                         m_filesDownloaded, m_totalFiles));
  m_progressBar->setTextureRect(
      {0, 0, (m_progressBarBG->getContentWidth() - 4.f) * progress,
       m_progressBar->getContentHeight()});
}

void DownloadPopup::startDownloadFile(std::filesystem::path const& path,
...
void DownloadPopup::handleFileDownloaded() {
  ...
  auto url = fmt::format(
      "https://raw.githubusercontent.com/EclipseMenu/EclipseMenu/refs/heads/"
      "main/resources/BitmapFonts/{}",
...
void DownloadPopup::handleError(int code) {
  m_progressLabel->setString(
      i18n::format("interface.font-download-fail", code));
  ...
  auto btn = geode::cocos::CCMenuItemExt::createSpriteExtra(
      ...
      [this](auto) { this->onClose(nullptr); });
