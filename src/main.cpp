...
class $modify(EclipseButtonMLHook, MenuLayer){
    bool init() override{if (!MenuLayer::init()) return false;
...
auto& key = keybinds::Manager::get()->registerKeybind("menu.toggle",
...
class HackUpdater : public cocos2d::CCObject {
 public:
  ...
  void update(float dt) override {
    ...
    if (s_isInitialized &&
        gui::Engine::getRendererType() == gui::RendererType::ImGui) {
      auto& io = ImGui::GetIO();
...
$on_mod(Loaded) {
  ...
  utils::get<cocos2d::CCFileUtils>()->addSearchPath(
      bmfontPath.string().c_str());
  geode::log::info("Added bitmap fonts to search path: {}",
                   bmfontPath.string());
  ...
  // Add "Interface" tab to edit theme settings
  {
    ...
    if (!themeNames.empty()) {
      auto themeCombo =
          tab->addCombo("interface.theme", "themeIndex", themeNames, 0);
      themeCombo->callback([](int value) {
        ThemeManager::get()->loadTheme(
            ThemeManager::get()->listAvailableThemes()[value].path);
    ...
    auto fontCombo = tab->addCombo("interface.font", "fontIndex",
                                   ThemeManager::getFontNames(), 0);
    fontCombo
        ->callback(
            [](int value) { ThemeManager::get()->setSelectedFont(value); })
        ->disableSaving();
    ...
    tab->addCombo("interface.layout-type", "layout",
                  {"Tabbed", "Panel", "Sidebar"}, 0)
        ->callback([](int value) {
          ThemeManager::get()->setLayoutMode(
              static_cast<imgui::LayoutMode>(value));
        })
        ->disableSaving();

    tab->addCombo("interface.style", "style", imgui::THEME_NAMES, 0)
        ->callback([](int value) {
          ThemeManager::get()->setComponentTheme(
              static_cast<imgui::ComponentTheme>(value));
        })
        ->disableSaving();

    auto blurToggle = tab->addToggle("interface.enable-blur", "blurEnabled")
                          ->callback([](bool value) {
                            ThemeManager::get()->setBlurEnabled(value);
                          });
    blurToggle->addOptions([](auto opt) {
      opt->addInputFloat("interface.blur-speed", "blurSpeed", 0.f, 10.f,
                         "%.3f s")
          ->callback(
              [](float value) { ThemeManager::get()->setBlurSpeed(value); })
          ->disableSaving();
    });
    blurToggle->disableSaving();

    auto accentColor =
        tab->addColorComponent("interface.accent-color", "accent", true);
    accentColor
        ->callback([](const Color& color) {
          ThemeManager::get()->applyAccentColor(color);
        })
        ->disableSaving();

    auto backgroundColor = tab->addColorComponent("interface.background-color",
                                                  "background", true);
    backgroundColor
        ->callback([](const Color& color) {
    ...
    languageCombo
        ->callback([languageCombo](int value) {
          if (Engine::getRendererType() == RendererType::Cocos2d) {
            ...
            if (i18n::hasBitmapFont(meta.charset)) {
              ...
              Popup::create(i18n::get_("common.warning"),
        ...
        ->callback([](std::string input) {
          ...
          if (input.empty()) {
            ...
            for (auto& tab : Engine::get()->getTabs()) {
              ...
              for (auto& component : tab->getComponents()) {
                if (utils::matchesStringFuzzy(i18n::get(component->getTitle()),
                                              input)) {
  ...
  cocos2d::CCScheduler::get()->scheduleSelector(
      schedule_selector(HackUpdater::update), HackUpdater::get(), 0.f, false);
}