...
class MixVideoAudioEvent : public geode::Event {
 public:
  ...
  std::filesystem::path const &getOutputMp4File() const {
...
class MixVideoRawEvent : public geode::Event {
 public:
  ...
  void setResult(const geode::Result<> &result) {
    m_result = geode::Result(result);
  }
  geode::Result<> getResult() { return m_result; }
...
class Recorder {
 public:
   ...
   *
   * @param settings The rendering settings that define the output
   * characteristics, including codec, bitrate, resolution, and pixel format.
   *
   ...
   *
   * This function ensures that all buffered frames are written to the output
   * file, releases allocated resources, and properly closes the output file.
   */
  void stop() { impl::StopRecorderEvent(m_ptr).post(); }
   ...
   *
   * @return true if the frame is successfully written, false if there is an
   * error.
   *
   * @warning Ensure that the frameData size matches the expected dimensions of
   * the frame.
   */
...
class AudioMixer {
 public:
   ...
   *
   * This function takes an input video file and an audio file, and merges them
   * into a single MP4 output file. The output MP4 file will have both the video
   * and audio streams synchronized.
   *
   ...
   *
   * @warning The audio file is expected to contain stereo (dual-channel) audio.
   * Using other formats might lead to unexpected results.
   ...
   *
   * This function takes an input video file and raw audio data (in the form of
   * a vector of floating-point samples), and merges them into a single MP4
   * output file.
   *
   * @param videoFile The path to the input video file.
   ...
   * @param outputMp4File The path where the output MP4 file will be saved.
   *
   * @warning The raw audio data is expected to be stereo (dual-channel). Using
   * mono or multi-channel audio might lead to issues.
   * @warning The video file is expected to contain a single video stream. Only
   * the first video stream will be copied.
   */
