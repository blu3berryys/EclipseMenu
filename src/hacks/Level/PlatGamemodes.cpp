...
class PlatGamemodes : public hack::Hack {
  ...
  [[nodiscard]] const char *getId() const override {
    return "All Modes in Platformer";
  }
};
...
class $modify(PlatGamemodesBGLHook, GJBaseGameLayer){
    ADD_HOOKS_DELEGATE("level.platgamemodes")
        ...
        void collisionCheckObjects(PlayerObject * player,
            ...
            if (!m_isPlatformer) return GJBaseGameLayer::collisionCheckObjects(
                player, sectionObjects, objectCount, dt);
