...
/// @brief Check whether hitboxes made by RobTop should be drawn. (e.g. in
/// practice mode)
inline bool robtopHitboxCheck() {
  if (auto *pl = utils::get<PlayLayer>())
    return pl->m_isPracticeMode &&
           utils::get<GameManager>()->getGameVariable("0166");
...
static std::deque<std::pair<cocos2d::CCRect, cocos2d::CCRect>> s_playerTrail1,
    s_playerTrail2;
...
class ShowHitboxes : public hack::Hack {
  void init() override {
    ...
    auto toggle = tab->addToggle("level.showhitboxes")
                      ->handleKeybinds()
                      ->setDescription();
    ...
    config::setIfEmpty("level.showhitboxes.solid_color",
                       gui::Color(0, 0.247, 1));
    ...
    config::setIfEmpty("level.showhitboxes.player_color_inner",
                       gui::Color(0, 1, 0.2f));
    config::setIfEmpty("level.showhitboxes.player_color_rotated",
                       gui::Color::YELLOW);
    ...
    toggle->addOptions([](std::shared_ptr<gui::MenuTab> options) {
      ...
      options->addToggle("level.showhitboxes.customcolors")
          ->addOptions([](std::shared_ptr<gui::MenuTab> optionsColor) {
            ...
            optionsColor->addColorComponent("level.showhitboxes.player_color",
                                            "level.showhitboxes.player_color");
            optionsColor->addColorComponent(
                "level.showhitboxes.player_color_inner",
                "level.showhitboxes.player_color_inner");
            optionsColor->addColorComponent(
                "level.showhitboxes.player_color_rotated",
                "level.showhitboxes.player_color_rotated");
          });
      options->addInputFloat("level.showhitboxes.bordersize",
                             "level.showhitboxes.bordersize", 0.01f, 10.f,
                             "%.2f");
      options->addFloatToggle("level.showhitboxes.fillalpha",
                              "level.showhitboxes.fillalpha", 0.f, 1.f);
      options->addFloatToggle("level.showhitboxes.traillength",
                              "level.showhitboxes.traillength", 1.f, 2000.f,
                              "%.0f");
    });
  }
...
enum HitboxType { Solid, Danger, Player, Other };
...
inline void drawRect(cocos2d::CCDrawNode *node, const cocos2d::CCRect &rect,
                     const gui::Color &color, float borderWidth,
  ...
  node->drawPolygon(vertices.data(), vertices.size(), color, borderWidth,
                    borderColor);
}

void drawForPlayer(cocos2d::CCDrawNode *node, PlayerObject *player,
                   const gui::Color &color, float borderWidth,
  ...
  drawRect(node, rect2, innerColor, borderWidth,
           {innerColor.r, innerColor.g, innerColor.b, 1.f});
}

void customDraw(cocos2d::CCDrawNode *drawNode, gui::Color &color,
                float &borderSize, gui::Color &borderColor) {
  if (s_skipDrawHook) {
  ...
  borderSize = hidePlayer ? 0.f
                          : config::get<float>("level.showhitboxes.bordersize",
                                               borderSize);
...
class $modify(ShowHitboxesCCDNHook, cocos2d::CCDrawNode){
    bool drawPolygon(cocos2d::CCPoint * vertex, unsigned int count,
                     const cocos2d::ccColor4F &fillColor, float borderWidth,
                     const cocos2d::ccColor4F &borderColor){
        borderWidth = abs(borderWidth);

customDraw(this, (gui::Color &)fillColor, borderWidth,
...
bool drawCircle(const cocos2d::CCPoint &position, float radius,
                ...
                const cocos2d::ccColor4F &borderColor, unsigned int segments) {
  borderWidth = abs(borderWidth);

  customDraw(this, (gui::Color &)color, borderWidth, (gui::Color &)borderColor);
  return CCDrawNode::drawCircle(position, radius, color, borderWidth,
                                borderColor, segments);
}
}
;
...
void forceDraw(GJBaseGameLayer *self, bool editor) {
  ...
  if (!config::get<bool>("level.showhitboxes.hideplayer", false)) {
    ...
    auto borderSize =
        config::get<float>("level.showhitboxes.bordersize", 0.25f);

    float alpha =
        config::get<bool>("level.showhitboxes.fillalpha.toggle", false)
            ? config::get<float>("level.showhitboxes.fillalpha", 0.25f)
            : 0.f;

    gui::Color playerColor =
        customColors
            ? config::get<gui::Color>("level.showhitboxes.player_color",
                                      gui::Color(1, 1, 0))
            : gui::Color(1, 0, 0, alpha);
    gui::Color playerColorInner =
        customColors
            ? config::get<gui::Color>("level.showhitboxes.player_color_inner",
                                      gui::Color(1, 1, 0))
            : gui::Color(0, 1, 0, alpha);
    playerColor.a = alpha;
    playerColorInner.a = alpha;

    drawForPlayer(self->m_debugDrawNode, self->m_player1, playerColor,
                  borderSize, playerColorInner);
    if (self->m_gameState.m_isDualMode)
      drawForPlayer(self->m_debugDrawNode, self->m_player2, playerColor,
                    borderSize, playerColorInner);

    if (config::get<bool>("level.showhitboxes.traillength.toggle", false)) {
      for (const auto &[rect, _] : s_playerTrail1)
        drawRect(self->m_debugDrawNode, rect, playerColor, borderSize,
      ...
      for (const auto &[_, rect] : s_playerTrail1)
        drawRect(
            self->m_debugDrawNode, rect, playerColorInner, borderSize,
...
class $modify(ShowHitboxesPOHook, PlayerObject){
    ENABLE_FIRST_HOOKS_ALL()
        ...
        void playerDestroyed(bool p0){if (auto *pl = utils::get<PlayLayer>())
                                          s_isDead = this == pl->m_player1 ||
                                                     this == pl->m_player2;
...
if (s_isDead ||
    !config::get<bool>("level.showhitboxes.traillength.toggle", false))
  return;

s_playerTrail1.emplace_back(m_player1->getObjectRect(),
                            m_player1->m_vehicleSize >= 1.f
                                ? m_player1->getObjectRect(0.25f, 0.25f)
                                : m_player1->getObjectRect(0.4f, 0.4f));

if (m_gameState.m_isDualMode) {
  s_playerTrail2.emplace_back(m_player2->getObjectRect(),
...
void updateDebugDraw() override {
  ...
  if (ptr1 == ptr2 && config::get<bool>("level.showhitboxes.editor", false))
    this->m_isDebugDrawEnabled |=
        config::get<bool>("level.showhitboxes", false);
...
class $modify(ShowHitboxesGOHook, GameObject){
    ENABLE_SAFE_HOOKS_ALL()
        ...
        void determineSlopeDirection(){
            /*
             * This is a fix for the slope hitbox becoming flipped during mirror
             * portal transition. I explained it in details for a Misc Bugfixes
             * PR: https://github.com/Cvolton/miscbugfixes-geode/pull/10
             */
