...
class $modify(ForcePlatformerGJBLHook, GJBaseGameLayer){
    ADD_HOOKS_DELEGATE("level.forceplatformer")
        ...
        void loadLevelSettings(){if (
            !utils::get<PlayLayer>() ||
            m_levelSettings
                ->m_platformerMode) return GJBaseGameLayer::loadLevelSettings();
...
class $modify(ForcePlatformerPLHook, PlayLayer){
    void resetLevel(){auto val = config ::get("level.forceplatformer", false);
