...
float getMultiplier(GameObject *gameObject) {
  ...
  else if (gameObject->m_objectType == GameObjectType::Hazard ||
           gameObject->m_objectType == GameObjectType::AnimatedHazard)
    return config::get<float>("level.hitbox_multiplier.hazard", 1.f);
  else if (gameObject->m_objectType == GameObjectType::Solid ||
           gameObject->m_objectType == GameObjectType::Slope)
...
void forceHitboxRecalculation() {
  for (GameObject *gameObject : s_affectedGameObjects) {
    ...
    gameObject->m_objectRadius =
        static_cast<HitboxGameObject *>(gameObject)->m_fields->m_originalRadius;
  }
  ...
  void init() override {
    ...
    tab->addToggle("level.hitbox_multiplier")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          options->addInputFloat("level.hitbox_multiplier.player",
                                 "level.hitbox_multiplier.player", 0.01f, 10.f,
                                 "%.2fx");
          options->addInputFloat("level.hitbox_multiplier.solid",
                                 "level.hitbox_multiplier.solid", 0.01f, 10.f,
                                 "%.2fx");
          options->addInputFloat("level.hitbox_multiplier.hazard",
                                 "level.hitbox_multiplier.hazard", 0.01f, 10.f,
                                 "%.2fx");
        });
  }

  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("level.hitbox_multiplier", false);
  }
  [[nodiscard]] const char *getId() const override {
    return "Hitbox Multiplier";
  }
};
...
class $modify(HitboxMultiplierLELHook, LevelEditorLayer){
    bool init(GJGameLevel * level, bool unk){reset();
...
class $modify(HitboxMultiplierOBB2DHook, OBB2D){
    void calculateWithCenter(cocos2d::CCPoint center, float width, float heigth,
                             float rotation){
        if (!utils::get<GJBaseGameLayer>() ||
            !config::get<bool>("level.hitbox_multiplier", false) ||
            !s_rotatedGameObject){s_rotatedGameObject = nullptr;
...
OBB2D::calculateWithCenter(center, width *multiplier, heigth *multiplier,
                           rotation);
