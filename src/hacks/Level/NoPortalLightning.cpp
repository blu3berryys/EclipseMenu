...
class $modify(NoPortalLightningGJBGLHook, GJBaseGameLayer){
    ALL_DELEGATES_AND_SAFE_PRIO("level.noportallightning")
        ...
        void lightningFlash(cocos2d::CCPoint from, cocos2d::CCPoint to,
                            cocos2d::ccColor3B color, float lineWidth,
                            float duration, int displacement, bool flash,
                            float opacity){auto* gm = utils::get<GameManager>();
