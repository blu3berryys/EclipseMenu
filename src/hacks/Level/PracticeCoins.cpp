...
class $modify(PracticeCoinsBGLHook, GJBaseGameLayer){
    ADD_HOOKS_DELEGATE("level.practicecoins")
        ...
        void collisionCheckObjects(PlayerObject * player,
            ...
            if (!m_isPracticeMode) return GJBaseGameLayer::
                collisionCheckObjects(player, sectionObjects, objectCount, dt);
...
for (int i = 0; i < objectCount; i++) {
  ...
  if (effectSprite->getOpacity() == 0) continue;
  ...
  effectSprite->EffectGameObject::triggerObject(this, player->m_uniqueID,
                                                nullptr);  // not sure if needed
  GJBaseGameLayer::destroyObject(
      effectSprite);  // plays the coin collect animation
  // if (!GJBaseGameLayer::hasUniqueCoin(effectSprite))
  //     GJBaseGameLayer::pickupItem(effectSprite); // not required, because the
  //     coin is not saved anyway
  GJBaseGameLayer::gameEventTriggered(GJGameEvent::UserCoin, 0,
                                      0);  // event triggers
}
