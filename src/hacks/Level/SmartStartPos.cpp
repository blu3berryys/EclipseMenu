...
class $modify(SmartStartPosPLHook, PlayLayer) {
  struct Fields {
    ...
    std::vector<GameObject *> m_dualPortals, m_gamemodePortals, m_miniPortals,
        m_speedChanges, m_mirrorPortals;
  };

  GameObject *getClosestObject(std::vector<GameObject *> & vec,
                               StartPosObject * startPos) {
  ...
  void setupStartPos(StartPosObject * startPos) {
    ...
    LevelSettingsObject *levelSettings =
        utils::get<PlayLayer>()->m_levelSettings;
