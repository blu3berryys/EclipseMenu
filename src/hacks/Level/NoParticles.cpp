...
void onHideParticles(bool state) {
  ...
  for (const auto &[name, array] :
       geode::cocos::CCDictionaryExt<gd::string, cocos2d::CCArray *>{
           gjbgl->m_particlesDict}) {
    for (const auto &particle :
         geode::cocos::CCArrayExt<cocos2d::CCParticleSystemQuad *>{array}) {
  ...
  for (const auto &particle :
       geode::cocos::CCArrayExt<cocos2d::CCParticleSystemQuad *>{
           gjbgl->m_unclaimedParticles}) {
    particle->setVisible(customParticlesEnabled);
  }

  for (const auto &[name, array] :
       geode::cocos::CCDictionaryExt<gd::string, cocos2d::CCArray *>{
           gjbgl->m_customParticles}) {
    for (const auto &particle :
         geode::cocos::CCArrayExt<cocos2d::CCParticleSystemQuad *>{array}) {
      particle->setVisible(customParticlesEnabled);
    }
  ...
  void init() override {
    ...
    tab->addToggle("level.noparticles")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          options->addToggle("level.noparticles.nomiscparticles")
              ->setDescription();
          options->addToggle("level.noparticles.nocustomparticles")
              ->setDescription();
        });
  }
...
class $modify(NoParticlesBGLHook, GJBaseGameLayer){
    ...
    spawnParticle(char const *plist, int zOrder,
                  cocos2d::tCCPositionType positionType,
                  cocos2d::CCPoint position){
        if (config::get<bool>("level.noparticles", false) &&
            config::get<bool>("level.noparticles.nomiscparticles",
