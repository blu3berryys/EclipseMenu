...
public:
  void init() override {
    ...
    config::setIfEmpty("label.startpos_switcher.color",
                       gui::Color(1.f, 1.f, 1.f, 0.6f));
    ...
    tab->addToggle("level.startpos_switcher")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          options
              ->addKeybind("level.startpos_switcher.previous",
                           "level.startpos_switcher.previous")
              ->setInternal();
          options
              ->addKeybind("level.startpos_switcher.next",
                           "level.startpos_switcher.next")
              ->setInternal();
          options->addToggle("level.startpos_switcher.reset_camera");
          options->addToggle("level.startpos_switcher.label")
              ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
                options->addInputFloat("label.startpos_switcher.scale",
                                       "label.startpos_switcher.scale", 0.1f,
                                       2.f, "%.2fx");
                options->addInputFloat("label.startpos_switcher.alpha_mod",
                                       "label.startpos_switcher.alpha_mod", 0.f,
                                       1.f);
                options->addColorComponent("label.startpos_switcher.color",
                                           "label.startpos_switcher.color",
                                           true);
                options->addToggle("label.startpos_switcher.buttons")
                    ->setDescription();
              });
        });
...
public:
  ...
  bool init(PlayLayer *playLayer) {
    ...
    m_next = CCMenuItemSpriteExtra::create(
        sprite, this, menu_selector(StartposSwitcherNode::onNext));
    sprite->setFlipX(true);
    m_next->setID("arrow-next");

    m_labelText =
        fmt::format("{}/{}", currentStartPosIndex + 1, startPosObjects.size());
    m_label =
        cocos2d::CCLabelBMFont::create(m_labelText.c_str(), "bigFont.fnt");
    m_label->setID("label");
    ...
    auto color = config::get<gui::Color>("label.startpos_switcher.color",
                                         gui::Color(1.f, 1.f, 1.f, 0.6f));
  ...
  void setVisibility(bool visible) {
    ...
    bool buttons =
        !visible ? false
                 : config::get<bool>("label.startpos_switcher.buttons", true);
    m_previous->setVisible(buttons);
    m_next->setVisible(buttons);
  }
  ...
  void update(float dt) override {
    ...
    if (m_timeSinceAction < 3.f) {
      ...
      auto opacityMod =
          1.f - config::get<float>("label.startpos_switcher.alpha_mod", 0.4f);
      auto clampedTime =
          std::clamp(m_timeSinceAction - 3.f, 0.f, animationTime) /
          animationTime;
...
void resetLevel() {
  ...
  // Reset camera
  if (currentStartPosIndex >= 0 &&
      config::get<bool>("level.startpos_switcher.reset_camera", true))
    this->resetCamera();
}
...
void addObject(GameObject *object) {
  ...
  if (object->m_objectID == 31)
    startPosObjects.push_back(
        geode::cast::typeinfo_cast<StartPosObject *>(object));
}
  ...
  if (m_startPosObject) {
    ...
    if (it != startPosObjects.end())
      currentStartPosIndex =
          static_cast<int32_t>(std::distance(startPosObjects.begin(), it));
  }
