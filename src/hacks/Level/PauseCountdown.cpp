  ...
  void init() override {
    ...
    tab->addToggle("level.pausecount")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          options->addInputFloat("level.pausecount.time",
                                 "level.pausecount.time", 0.1f, 15.f, "%.2fs");
        });
  }
...
protected:
  ...
  bool init() override {
    if (!CCLayerColor::initWithColor(ccc4(50, 50, 50, 100)))
      return false;
    m_totaldt = config::get<float>("level.pausecount.time", 3.f);
    m_countdown = CCLabelBMFont::create(
        std::to_string(static_cast<int>(m_totaldt)).c_str(), "goldFont.fnt");
  ...
  void updateTimer(float dt) {
    ...
    m_countdown->setString(
        std::to_string(static_cast<int>(m_totaldt) + 1).c_str());
  }
};
...
class $modify(PauseBGLHook, GJBaseGameLayer) {
  ...
  void update(float dt) {
    ...
    if (m_fields->pausedt > 0.f)
      m_fields->pausedt -= dt;
    if (m_fields->pausedt <= 0.01f ||
        !config::get<bool>("level.pausecount", false))
      GJBaseGameLayer::update(dt);
  }
};

class $modify(PauseCountdownPlayLHook, PlayLayer){void pauseGame(bool paused){
    if (paused){if (static_cast<PauseBGLHook *>(utils::get<GJBaseGameLayer>())
                            ->m_fields->pausedt < 0.01f ||
                    !config::get<bool>("level.pausecount", false))
  ...
  if (!utils::get<PlayLayer>() || !bg)
    return;
  if (bg->m_gameState.m_currentProgress == 0)
    return;
