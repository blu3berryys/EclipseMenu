...
class $modify(LabelsPOHook, PlayerObject){bool pushButton(PlayerButton btn){
...
class $modify(LabelsPLHook,
              PlayLayer){bool init(GJGameLevel * level, bool unk1, bool unk2){
...
if (this->getChildrenCount()) {
  ...
  for (auto *child : children) {
    ...
    if (auto *label =
            geode::cast::typeinfo_cast<cocos2d::CCLabelBMFont *>(child);
        label && label->getString() == "Testmode"sv) {
...
class $modify(LabelsUILHook, UILayer) {
  struct Fields {
    ...
    std::vector<std::pair<SmartLabel *, std::function<void(SmartLabel *)>>>
        m_absoluteLabels;
  ...
  void createLabels() {
    ...
    for (auto &container : fields->m_containers) {
      ...
      container->setID(
          fmt::format("label-container-{}", static_cast<int>(alignment)));
      fields->m_mainContainer->addChild(container);
      alignment = static_cast<LabelsContainer::Alignment>(
    ...
    // Add the cheat indicator
    {
      ...
      auto *container = fields->m_containers[config::get<int>(
          "labels.cheat-indicator.alignment", 0)];
      container->addLabel(cheatIndicator, [](SmartLabel *label) {
        bool visible =
            config::get<bool>("labels.cheat-indicator.visible", false);
        ...
        bool showOnlyCheating =
            config::get<bool>("labels.cheat-indicator.only-cheating", false);
        ...
        label->setScale(
            config::get<float>("labels.cheat-indicator.scale", 0.5f));
        label->setOpacity(static_cast<GLubyte>(
            config::get<float>("labels.cheat-indicator.opacity", 0.35f) * 255));

        // Cheating - Red, Tripped - Orange, Normal - Green
        auto color = isCheating   ? gui::Color::RED
                     : hasTripped ? gui::Color{0.72f, 0.37f, 0.f}
                                  : gui::Color::GREEN;
      ...
      for (auto &setting : s_labels) {
        ...
        if (setting.absolutePosition) {
          ...
          fields->m_absoluteLabels.emplace_back(
              label, [&setting](SmartLabel *label) {
          ...
          auto *container =
              fields->m_containers[static_cast<int>(setting.alignment)];
  ...
  bool init(GJBaseGameLayer * bgl) {
    ...
    auto fields = m_fields.self();
    fields->m_isEditor =
        geode::cast::typeinfo_cast<LevelEditorLayer *>(bgl) != nullptr;
...
class Labels : public hack::Hack {
    ...
    tab->addToggle("labels.cheat-indicator.visible")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          ...
          options->addToggle("labels.cheat-indicator.only-cheating")
              ->setDescription();
          options
              ->addInputFloat("labels.cheat-indicator.scale",
                              "labels.cheat-indicator.scale", 0.1f, 2.f, "%.1f")
              ->setDescription()
              ->callback([](float) { updateLabels(); });
          options
              ->addInputFloat("labels.cheat-indicator.opacity",
                              "labels.cheat-indicator.opacity", 0.f, 1.f,
                              "%.2f")
              ->setDescription();
          options
              ->addCombo("labels.cheat-indicator.alignment",
                         "labels.cheat-indicator.alignment",
                         {
        ...
        if (auto value = event->getValue()) {
          ...
          gui::Engine::queueAfterDrawing([this, path] {
            ...
            if (json.is_discarded()) {
              return Popup::create(i18n::get_("labels.import-failed"),
  ...
  void createLabelComponent() {
    ...
    for (auto &setting : s_labels) {
      ...
      toggle
          ->deleteCallback([this, &setting] {
            Popup::create(i18n::get_("labels.delete-prompt"),
                          ...
                          [this, &setting](bool yes) {
                            ...
                            gui::Engine::queueAfterDrawing([&] {
                              auto it = std::ranges::find_if(
                                  s_labels,
                                  [&setting](const labels::LabelSettings &s) {
          ...
          ->exportCallback([&setting] { setting.promptSave(); })
          ->moveCallback([this, &setting](bool up) {
            ...
            // refresh ui
            gui::Engine::queueAfterDrawing([this] { createLabelComponent(); });
          })
          ->handleKeybinds();
