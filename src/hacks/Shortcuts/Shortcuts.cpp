...
class Shortcuts : public hack::Hack {
  ...
  static void uncompleteLevel() {
    ...
    Popup::create(
        ...
        i18n::get_("common.yes"), i18n::get_("common.no"), [level](bool yes) {
          ...
          // removed if the said level was rated
          if (level->m_normalPercent >= 100 && gsm->hasCompletedLevel(level)) {
            ...
            if (level->m_stars > 0) {
              // GSM->m_completedLevels->removeObjectForKey(fmt::format("unique_{}",
              // levelid));
              gsm->m_completedLevels->removeObjectForKey(
                  fmt::format("star_{}", levelid));
              gsm->m_completedLevels->removeObjectForKey(
  ...
  static void injectDll() {
    ...
    m_listener.bind([](FileEvent::Event *event) {
      if (auto value = event->getValue()) {
        ...
        if (success) {
          ...
          auto dllMain =
              reinterpret_cast<DllMain>(GetProcAddress(module, "DllMain"));
          if (dllMain)
            dllMain(static_cast<HINSTANCE>(module), DLL_PROCESS_ATTACH,
                    nullptr);
        } else {
          FreeLibrary(module);
    ...
    m_listener.setFilter(
        geode::utils::file::pick(geode::utils::file::PickMode::OpenFile,
  ...
  void init() override {
    ...
    tab->addButton("shortcuts.options")
        ->setDescription()
        ->callback(openSettings)
        ->handleKeybinds();
    tab->addButton("shortcuts.uncomplete-level")
        ->setDescription()
        ->callback(uncompleteLevel)
        ->handleKeybinds();
    tab->addButton("shortcuts.restart-level")
        ->setDescription()
        ->callback(restartLevel)
        ->handleKeybinds();
    tab->addButton("shortcuts.toggle-practice")
        ->setDescription()
        ->callback(togglePracticeMode)
        ->handleKeybinds();
    tab->addButton("shortcuts.place-checkpoint")
        ->setDescription()
        ->callback(placeCheckpoint)
        ->handleKeybinds();
    tab->addButton("shortcuts.remove-checkpoint")
        ->setDescription()
        ->callback(removeCheckpoint)
        ->handleKeybinds();
    GEODE_WINDOWS(tab->addButton("shortcuts.inject-dll")
                      ->setDescription()
                      ->callback(injectDll)
                      ->handleKeybinds();)
    tab->addButton("shortcuts.save-folder")
        ->setDescription()
        ->callback(openSaveFolder)
        ->handleKeybinds();
