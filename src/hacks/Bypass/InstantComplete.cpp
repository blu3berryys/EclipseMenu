...
class InstantComplete : public hack::Hack {
  void init() override {
    ...
    tab->addToggle("bypass.instantcomplete")
        ->handleKeybinds()
        ->setDescription();
  }

  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("bypass.instantcomplete", false);
  }
  [[nodiscard]] const char *getId() const override {
    return "Instant Complete";
  }
};

REGISTER_HACK(InstantComplete)

class $modify(InstantCompletePLHook,
              PlayLayer){ADD_HOOKS_DELEGATE("bypass.instantcomplete")
