...
class UnlockMainLevels : public hack::Hack {
  void init() override {
    ...
    tab->addToggle("bypass.unlockmainlevels")
        ->setDescription()
        ->handleKeybinds();
  }

  [[nodiscard]] const char *getId() const override {
    return "Unlock Main Levels";
  }
};
...
class $modify(UnlockMainLevelsGJHook, GameLevelManager){
    ENABLE_SAFE_HOOKS_ALL()
        ...
        GJGameLevel *
    getMainLevel(int levelID, bool dontGetLevelString){
        auto level = GameLevelManager::getMainLevel(levelID,
                                                    dontGetLevelString);
if (level->m_requiredCoins > 0 &&
    config::get<bool>("bypass.unlockmainlevels", false)) {
