...
class $modify(UnlockVaultsGMHook, GameManager){
    ADD_HOOKS_DELEGATE("bypass.unlockvaults")
        ...
        bool getUGV(const char* key){bool result = GameManager::getUGV(key);
if (s_bypassUGVSkip) {
...
class $modify(UnlockVaultsGSMHook, GameStatsManager){
    ADD_HOOKS_DELEGATE("bypass.unlockvaults")
        ...
        int getStat(const char* key){int value = GameStatsManager::getStat(key);
...
class $modify(UnlockVaultsCLHook, CreatorLayer){
    ADD_HOOKS_DELEGATE("bypass.unlockvaults")
        ...
        void onSecretVault(cocos2d::CCObject * sender){s_bypassGameStat = true;
...
class $modify(UnlockVaultsSL2Hook, SecretLayer2){
    ADD_HOOKS_DELEGATE("bypass.unlockvaults")
        ...
        void onDoor(cocos2d::CCObject * sender){s_bypassUGV = true;
SecretLayer2::onDoor(sender);
}
}
;

class $modify(UnlockVaultsLPHook,
              LevelPage){void addSecretDoor(){s_bypassUGV = true;
