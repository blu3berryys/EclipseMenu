  ...
  Popup::prompt(
      ...
      [&](bool result, std::string name) {
        ...
        config::set("bot.selectedreplay",
                    Mod::get()->getSaveDir() / "replays" / (name + ".gdr"));
      },
      i18n::get_("common.create"), i18n::get_("common.cancel"), "");
}
        ...
        i18n::get_("common.yes"), i18n::get_("common.no"), [&](bool result) {
          ...
          Popup::create(
              ...
              i18n::format("bot.saved.msg",
                           confirmReplayDirectory.filename().stem().string(),
                           s_bot.getInputCount()));
...
void confirmLoad(std::filesystem::path const &replayPath) {
  ...
  if (res.isErr()) {
    return Popup::create(i18n::get_("common.error"),
                         i18n::format("bot.load-fail", res.unwrapErr()));
  }
  Popup::create(i18n::get_("bot.loaded"),
                i18n::format("bot.loaded.msg", replayPath.filename().stem(),
                             s_bot.getInputCount()));
}
...
void deleteReplay() {
  std::filesystem::path replayPath =
      config::get<std::string>("bot.selectedreplay", "");
  if (!std::filesystem::exists(replayPath)) {
    return Popup::create(i18n::get_("bot.delete-invalid"),
  ...
  Popup::create(i18n::get_("common.warning"),
                ...
                [replayPath, replayName](bool result) {
                  ...
                  if (std::filesystem::exists(replayPath)) {
                    ...
                    // apparently i cannot put the Popup below here otherwise
                    // some memory corruption happens, WHY? its not even a
                    // pointer!!
                    config::set("bot.selectedreplay", "");
                  }
                });
...
class Bot : public hack::Hack {
  void init() override {
    ...
    tab->addRadioButton("bot.disabled", "bot.state", 0)
        ->callback(updateBotState)
        ->handleKeybinds();
    tab->addRadioButton("bot.record", "bot.state", 1)
        ->callback(updateBotState)
        ->handleKeybinds();
    tab->addRadioButton("bot.playback", "bot.state", 2)
        ->callback(updateBotState)
        ->handleKeybinds();

    tab->addFilesystemCombo("bot.replays", "bot.selectedreplay",
                            Mod::get()->getSaveDir() / "replays");
...
class $modify(BotPLHook,
              PlayLayer){bool init(GJGameLevel * gj, bool p1, bool p2){
...
void resetLevel() {
  ...
  if (s_bot.getState() == bot::State::RECORD) {
    // gd does this automatically for holding but not releases so we do it
    // manually
...
CheckpointObject *markCheckpoint() {
  if (s_bot.getState() == bot::State::RECORD &&
      (m_player1->m_isDead || m_player2->m_isDead))
    return nullptr;
...
while ((input = s_bot.poll(m_gameState.m_currentProgress)) != std::nullopt) {
  GJBaseGameLayer::handleButton(input->down, (int)input->button,
                                !input->player2);
}
}
...
void handleButton(bool down, int button, bool player1) {
  ...
  s_bot.recordInput(m_gameState.m_currentProgress, (PlayerButton)button,
                    !realPlayer1, down);
}
}
;
