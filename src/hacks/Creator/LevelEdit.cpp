...
class $modify(LevelEditPLHook, PauseLayer){
    ALL_DELEGATES_AND_SAFE_PRIO("creator.leveledit")
        ...
        void customSetup()
            override{auto level = utils::get<PlayLayer>()->m_level;
...
class $modify(LevelEditEUIHook, EditorUI){void onSettings(CCObject * sender){
...
class $modify(LevelEditEPLHook, EditorPauseLayer){
    void customSetup() override{auto level = this->m_editorLayer->m_level;
auto levelType = level->m_levelType;
level->m_levelType = GJLevelType::Editor;
...
class $modify(LevelEditLTHook, LevelTools){
    ENABLE_SAFE_HOOKS_ALL()
        ...
        static bool verifyLevelIntegrity(gd::string levelString, int levelID){
            if (LevelTools::verifyLevelIntegrity(std::move(levelString),
                                                 levelID)) return true;
