  ...
  void init() override {
    ...
    tab->addToggle("player.framestepper")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          options->addKeybind("player.framestepper.step_key",
                              "player.framestepper.step_key");
          options->addToggle("player.framestepper.hold",
                             "player.framestepper.hold");
          options->addInputFloat("player.framestepper.hold_delay",
                                 "player.framestepper.hold_delay", 0.0f,
                                 FLT_MAX, "%.2f");
          options->addInputInt("player.framestepper.hold_speed",
                               "player.framestepper.hold_speed", 0);
        });
  }

  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("player.framestepper", false);
  }
...
class HoldingMenuItem : public CCMenuItemSpriteExtra {
 public:
  ...
  bool init(cocos2d::CCSprite *sprite, const std::function<void()> &onHold,
            const std::function<void()> &onRelease) {
...
class $modify(FrameSFrameStepperUILHook, UILayer){
    bool init(GJBaseGameLayer * bgl){if (!UILayer::init(bgl)) return false;
