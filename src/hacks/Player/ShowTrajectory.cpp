...
class TrajectorySimulation {
 ...
 private:
  void drawRectangleHitbox(const cocos2d::CCRect &rect, const gui::Color &color,
    ...
    getDrawNode()->drawPolygon(vertices.data(), vertices.size(),
                               {color.r, color.g, color.b, 0}, 0.25f,
                               borderColor);
  }
  ...
  void drawForPlayer(PlayerObject *player) const {
    ...
    auto color = config::get<gui::Color>("level.showhitboxes.player_color",
                                         {1.f, 0, 0, 1.f});
    auto colorInner = config::get<gui::Color>(
        "level.showhitboxes.player_color_inner", {0, 1.f, 0, 1.f});
  ...
  void iterateForPlayer(PlayerObject *player, bool down, bool isPlayer2) {
    ...
    const size_t iterations =
        config::get<int>("player.showtrajectory.iterations", 300);
    ...
    for (size_t i = 0; i < iterations; i++) {
      ...
      if (!iterationActionDone &&
        ...
        down ? player->pushButton(PlayerButton::Jump)
      ...
      getDrawNode()->drawSegment(
          initialPlayerPosition, player->getPosition(), 0.65f,
          down ? cocos2d::ccColor4F{0.f, 1.f, 0.1f, 1.f}
               : cocos2d::ccColor4F{1.f, 0.f, 0.1f, 1.f});
    }
  ...
  void simulateForPlayer(PlayerObject *player, PlayerObject *player2,
 ...
 public:
  ...
  void hide() const { getDrawNode()->setVisible(false); }
...
class ShowTrajectory : public hack::Hack {
  void init() override {
    ...
    toggle->addOptions([](std::shared_ptr<gui::MenuTab> options) {
      options->addInputInt("player.showtrajectory.iterations",
                           "player.showtrajectory.iterations", 1, 1000);
    });
  ...
  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("player.showtrajectory", false);
  }
...
void destroyPlayer(PlayerObject *player, GameObject *gameObject) override {
  if (gameObject != m_anticheatSpike && s_simulation.isSimulationDead(player))
    return;
...
class $modify(ShowTrajectoryLELHook,
              LevelEditorLayer){bool init(GJGameLevel * level, bool unk){
...
class $modify(ShowTrajectoryHSHook,
              HardStreak){ADD_HOOKS_DELEGATE("player.showtrajectory")
...
class $modify(ShowTrajectoryEGOHook, EffectGameObject){
    ADD_HOOKS_DELEGATE("player.showtrajectory")
        ...
        void triggerObject(GJBaseGameLayer * p0, int p1,
                           const gd::vector<int> *p2)
            override{if (s_simulation.isSimulating()) return;
...
class $modify(ShowTrajectoryGOHook, GameObject){
    ADD_HOOKS_DELEGATE("player.showtrajectory")
        ...
        void playShineEffect(){if (s_simulation.isSimulating()) return;
...
class $modify(ShowTrajectoryBGLHook, GJBaseGameLayer){
    ADD_HOOKS_DELEGATE("player.showtrajectory")
        ...
        bool canBeActivatedByPlayer(PlayerObject * p0, EffectGameObject *p1){
            if (s_simulation.isSimulating()) return false;
