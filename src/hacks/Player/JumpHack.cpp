...
class JumpHack : public hack::Hack {
  ...
  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("player.jumphack", false);
  }
...
class $modify(JumpHackPOHook,
              PlayerObject){ADD_HOOKS_DELEGATE("player.jumphack")

                                bool pushButton(PlayerButton p0){jump = true;
