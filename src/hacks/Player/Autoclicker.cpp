  ...
  void init() override {
    ...
    tab->addToggle("player.autoclick")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          ...
          options->addInputInt("player.autoclick.intervalhold",
                               "player.autoclick.intervalhold", 1, 1000);
          options->addInputInt("player.autoclick.intervalrelease",
                               "player.autoclick.intervalrelease", 1, 1000);
        });
  }

  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("player.autoclick", false);
  }
...
class $modify(AutoClickerBGLHook, GJBaseGameLayer) {
  ...
  void processCommands(float dt) {
    ...
    auto releaseInterval =
        config::get<int>("player.autoclick.intervalrelease", 1);
    m_fields->timer++;
    if ((m_fields->timer >= clickInterval && !m_fields->clicking) ||
        (m_fields->timer >= releaseInterval && m_fields->clicking)) {
      m_fields->clicking = !m_fields->clicking;
      if (config::get<bool>("player.autoclick.p1"))
        this->handleButton(m_fields->clicking, 1, true);
      if (config::get<bool>("player.autoclick.p2"))
        this->handleButton(m_fields->clicking, 1, false);
      m_fields->timer = 0.f;
    }
  }
};
