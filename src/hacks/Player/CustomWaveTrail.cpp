  ...
  void init() override {
    ...
    tab->addToggle("player.customwavetrail")
        ...
        ->addOptions([](auto options) {
          options->addInputFloat("player.customwavetrail.scale",
                                 "player.customwavetrail.scale", 0.f, 10.f,
                                 "%.2f");
          options->addToggle("player.customwavetrail.rainbow")
              ->addOptions([](auto opt) {
                opt->addInputFloat("player.customwavetrail.speed",
                                   "player.customwavetrail.speed", 0.f, FLT_MAX,
                                   "%.2f");
                opt->addInputFloat("player.customwavetrail.saturation",
                                   "player.customwavetrail.saturation", 0.f,
                                   100.f, "%.2f");
                opt->addInputFloat("player.customwavetrail.value",
                                   "player.customwavetrail.value", 0.f, 100.f,
                                   "%.2f");
              });
          options->addToggle("player.customwavetrail.customcolor")
              ->addOptions([](auto opt) {
                opt->addColorComponent("player.customwavetrail.color",
                                       "player.customwavetrail.color");
              });
        });
  }

  [[nodiscard]] const char* getId() const override {
    return "Custom Wave Trail";
  }
};
    ...
    ADD_HOOKS_DELEGATE("player.customwavetrail")
        ...
        void updateStroke(float dt){
            if (config::get<bool>("player.customwavetrail.rainbow", false)){
                auto speed = config::get<float>("player.customwavetrail.speed",
                                                0.5f);
auto saturation =
    config::get<float>("player.customwavetrail.saturation", 100.f);
