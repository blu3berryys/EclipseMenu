...
class Noclip : public hack::Hack {
  void init() override {
    ...
    tab->addToggle("player.noclip")
        ...
        ->addOptions([](std::shared_ptr<gui::MenuTab> options) {
          ...
          options
              ->addFloatToggle("player.noclip.acclimit",
                               "player.noclip.acclimit", 0.01f, 100.f, "%.2f")
              ->handleKeybinds();
          options
              ->addIntToggle("player.noclip.deathlimit",
                             "player.noclip.deathlimit", 1, 100)
              ->handleKeybinds();
          options->addToggle("player.noclip.tint");
          options->addColorComponent("player.noclip.color",
                                     "player.noclip.color");
          options->addInputFloat("player.noclip.opacity",
                                 "player.noclip.opacity", 0.f, 100.f, "%.0f%");
          options
              ->addInputFloat("player.noclip.time", "player.noclip.time", 0.f,
                              5.f, "%.2fs")
              ->setDescription();
        });
  }
  ...
  [[nodiscard]] bool isCheating() override {
    return config::get<bool>("player.noclip", false);
  }
...
class $modify(NoClipPLHook, PlayLayer) {
  ...
  void destroyPlayer(PlayerObject * player, GameObject * object) override {
    ...
    if (!fields->m_noclipTint &&
        config::get<bool>("player.noclip.tint", false)) {
      auto color =
          config::get<gui::Color>("player.noclip.color", gui::Color::RED)
              .toCCColor3B();
    ...
    bool player2 =
        config::get<bool>("player.noclip.p2", true) && player == m_player2;
    if (player1 || player2) {
  ...
  void postUpdate(float dt) override {
    auto fields = m_fields.self();
    config::setTemp<bool>("noclipDying",
                          fields->m_wouldDieFrame || fields->m_deadLastFrame);
        ...
        if (fields->m_wouldDie) {
          if (fields->m_tintOpacity < 1.f) {
            ...
            fields->m_noclipTint->setOpacity(
                ...
                config::get<float>("player.noclip.opacity", 90.f));
          }
...
if (fields->m_wouldDieFrame) {
  ...
  if (!fields->m_deadLastFrame) {
    ...
    if (config::get<bool>("player.noclip.deathlimit.toggle", false) &&
        deaths + 1 >= config::get<int>("player.noclip.deathlimit", 0))
...
if (frame > 0) {
  float acc = static_cast<float>(frame - fields->m_deadFrames) /
              static_cast<float>(frame) * 100.f;
  config::setTemp("noclipAccuracy", acc);
  bool dead =
      (m_player1 && m_player1->m_isDead) || (m_player2 && m_player2->m_isDead);
  // if (config::get<bool>("player.noclip.acclimit.toggle", false) && acc <=
...
/*
if (config::get<bool>("player.noclip.acclimit.toggle", false) && acc <
config::get<float>("player.noclip.acclimit", 95.f)) {
                    utils::get<PlayLayer>()->destroyPlayer(m_player1, nullptr);
                    config::setTemp("noclipAccuracy",
config::get<float>("player.noclip.acclimit", 95.f));
                }*/