...
class RespawnDelay : public hack::Hack {
  void init() override {
    ...
    tab->addFloatToggle("player.respawndelay", "player.respawndelay", 0.f,
                        120.f, "%.2f s.")
        ->handleKeybinds();
  }
...
class $modify(RespawnDelayPLHook, PlayLayer){
    ALL_DELEGATES_AND_SAFE_PRIO("player.respawndelay.toggle")
        ...
        void destroyPlayer(PlayerObject * player, GameObject *object)
            override{PlayLayer::destroyPlayer(player, object);
...
if (auto *respawnSequence = this->getActionByTag(0x10)) {
  ...
  auto *newSequence = cocos2d::CCSequence::create(
      ...
      cocos2d::CCCallFunc::create(
          this, callfunc_selector(PlayLayer::delayedResetLevel)),
      nullptr);
