...
class TPSBypass : public hack::Hack {
 public:
  void init() override {
    ...
    tab->addFloatToggle("global.tpsbypass", "global.tpsbypass", MIN_TPS,
                        MAX_TPS, "%.2f TPS")
        ->setDescription()
        ->handleKeybinds();
  }
...
class $modify(TPSBypassGJBGLHook, GJBaseGameLayer) {
  ...
  void update(float dt) override {
    ...
    if (fields->m_extraDelta >= newDelta) {
      // call original update several times, until the extra delta is less than
      // the new delta
...
void levelComplete() {
  ...
  // also we can't rely on m_level->m_timestamp, because it might not be updated
  // yet
...
class $modify(TPSBypassLELHook, LevelEditorLayer){
    ...
    // Editor postUpdate handles the exit of playback mode and player trail
    // drawing and calls updateVisibility
    void postUpdate(float dt)
        override{// editor disables playback mode in postUpdate, so we should
                 // call the function if we're dead
                 auto fields = getFields(this);
if (fields->m_shouldHide && !m_player1->m_maybeIsColliding &&
    !m_player2->m_maybeIsColliding)
  return;
