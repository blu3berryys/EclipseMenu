...
class LockCursor : public hack::Hack {
  ...
  void update() override {
    ...
    if (pl->m_hasCompletedLevel || pl->m_isPaused)
      return; // level is completed or paused
    if (!config::get<bool>("global.lockcursor", false))
      return; // not toggled
    if (gui::Engine::get()->isToggled())
      return; // gui is open

    GEODE_WINDOWS(HWND hwnd = WindowFromDC(wglGetCurrentDC()); RECT winSize;
    ...
    GEODE_MACOS(CGEventRef ourEvent = CGEventCreate(NULL);
                auto point = CGEventGetLocation(ourEvent); CFRelease(ourEvent);
