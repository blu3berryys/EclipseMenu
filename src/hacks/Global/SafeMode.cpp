 ...
 public:
  static bool hasCheats() {
    ...
    return std::ranges::any_of(hacks,
                               [](auto& hack) { return hack->isCheating(); });
  }
...
class $modify(SafeModePLHook, PlayLayer) {
  ...
  void resetLevel() {
    ...
    if ((safeMode || AutoSafeMode::shouldEnable()) &&
        config::get<bool>("global.safemode.freeze_attempts", true))
...
class $modify(SafeModePOHook, PlayerObject){
    ENABLE_SAFE_HOOKS_ALL()
        ...
        void incrementJumps(){if (
            config::get<bool>("global.safemode", false) ||
