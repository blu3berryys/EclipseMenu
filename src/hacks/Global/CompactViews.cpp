...
class CompactProfileComments : public hack::Hack {
  ...
  [[nodiscard]] const char *getId() const override {
    return "Compact Profile Comments";
  }
};
...
class $modify(CompactLevelsCommentsCLVHook, CustomListView){
    ...
    proof of consent of code reuse:
    https://discord.com/channels/911701438269386882/911702535373475870/1220117410988953762
...
class $modify(CompactLevelsCommentsLCHook,
              LevelCell){void onClick(CCObject * sender){
    // get the "view" button to work with compact mode in "my levels"
    if (this->m_level->m_levelType == GJLevelType::Editor &&
        config::get<bool>("global.compacteditorlevels", false))
        utils::get<cocos2d::CCDirector>()
            ->replaceScene(cocos2d::CCTransitionFade::create(
                0.5f, EditLevelLayer::scene(m_level)));
...
void loadLocalLevelCell() {
  ...
  if (config::get<bool>("global.compacteditorlevels", false) && m_mainLayer) {
    ...
    if (const auto localLevelName =
            m_mainLayer->getChildByType<cocos2d::CCLabelBMFont>(0))
      if (std::string(localLevelName->getString()) ==
