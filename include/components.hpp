...
inline components::Toggle MenuTab::addModSettingToggle(
    std::shared_ptr<geode::Setting> const& setting) const {
  ...
  // Add the toggle
  auto toggle = addToggle(mod->expandSpriteName(settingId).data(), title,
                          [mod, settingId](bool v) {
  ...
  geode::listenForSettingChanges<bool>(
      settingId, [toggle](bool v) { config::set(toggle.getID(), v); }, mod);
